...
/**
 * The ITypeProvider interface defines constants used for type computation and resolution in the GAMA modeling platform.
 * 
 * <p>This interface provides a comprehensive set of constants that enable dynamic type determination based on 
 * expression arguments, container properties, and contextual relationships. It is primarily used by the type 
 * system to resolve types at compile-time and runtime.</p>
 * 
 * <p>The constants fall into several categories:</p>
 * <ul>
 *   <li><strong>Index-based constants:</strong> Determine types based on argument positions (TYPE_AT_INDEX, CONTENT_TYPE_AT_INDEX, etc.)</li>
 *   <li><strong>Relationship constants:</strong> Define types based on owner/container relationships (OWNER_TYPE, MACRO_TYPE, etc.)</li>
 *   <li><strong>Special type constants:</strong> Handle specific type resolution scenarios (BOTH, ALL, NONE, etc.)</li>
 *   <li><strong>Container-aware constants:</strong> Work with container types and their contents (KEY_TYPE_AT_INDEX, etc.)</li>
 * </ul>
 * 
 * @author drogoul
 * @since 2.0 (Modified on 2 août 2010)
 * @see IType
 */
public interface ITypeProvider {
	/**
	 * Internal threshold value used to distinguish fixed constants from the "index-based" constants below.
	 * All index-based constants have values below this threshold.
	 */
	int INDEXED_TYPES = -100;
	/**
	 * The type returned is the type of the argument at index 1...n (i.e., 1 for the first element, 2 for the
	 * second, etc.) so that, for instance, TYPE_AT_INDEX + 2 represents the type of the SECOND argument (equivalent to
	 * SECOND_TYPE). Must be followed by "+ n" with n>0 to be significant.
	 */
	...
	int MODEL_TYPE = -27;
	/**
	 * The generic type representing an experiment (which does not exist as a concrete type -- at least not yet)
	 */
	 ...
	 * EXPERIMENTAL RIGHT NOW (and probably limited to the matrix and as_matrix operators).
	 * Example: matrix([[4, 5, 6], [1, 2, 4]]) should get int as a content type.
	 */
