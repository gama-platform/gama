...
public abstract class AbstractGamlAdditions extends UtilsForGamlAdditions implements IGamlAdditions {
	...
	protected void _type(final String keyword, final IType<?> typeInstance, final int id, final int varKind,
			final Class<?>... wraps) {
		final IType<?> type = builtInTypes.initType(keyword, (IType) typeInstance, id, varKind, (Class) wraps[0],
				CURRENT_PLUGIN_NAME);
		// Optimize loop: cache type name and avoid repeated method calls
		final String typeName = type.getName();
		for (final Class<?> cc : wraps) { Types.addClassTypeCorrespondance(cc, typeName); }
	...
	protected void _skill(final String name, final Class<?> clazz, final String... species) {
		final Iterable<IDescription> children = GAML.ADDITIONS.get(clazz);
		if (children != null) {
		...
		GamaSkillRegistry.INSTANCE.register(sd, (Class) clazz);
	...
	public void _operator(final String[] keywords, final Executable method, final String doc,
		...
		final int nbParameters = signature.size();
		final String plugin = CURRENT_PLUGIN_NAME;
		final IType<?> rt = returnClassOrType instanceof Class ? Types.get((Class<?>) returnClassOrType)
				: (IType<?>) returnClassOrType;

		// Pre-compute agent type check for optimization
		final boolean isAgentTypeSignature = nbParameters == 2 && signature.get(0).isAgentType();

		for (final String keyword : keywords) {
			// Optimize map access - computeIfAbsent creates map only if needed
			final Map<Signature, IArtefactProto.Operator> map =
					GAML.OPERATORS.computeIfAbsent(keyword, k -> new HashMap<>());

			if (!map.containsKey(signature)) {
				final boolean isSpecialOperator = isAgentTypeSignature && (OF.equals(keyword) || _DOT.equals(keyword));
				final IArtefactProto.Operator proto = GAML.getArtefactProtoFactory().createOperatorProto(keyword,
						method, isSpecialOperator ? null : doc, helper, c, isSpecialOperator, rt, signature, t, content,
						index, contentContentType, expectedContentTypes, plugin);
	...
	protected void _var(final Class<?> clazz, final IDescription desc, final IGamaHelper get, final IGamaHelper init,
		...
		vd.setDefiningPlugin(CURRENT_PLUGIN_NAME);
	...
	protected void _field(final Class<?> clazz, final String name, final IGamaGetter helper, final int returnType,
			final Class<?> signature, final int typeProvider, final int contentTypeProvider,
			final int keyTypeProvider) {
	...
	protected void _action(final IGamaHelper e, final IDescription desc, final Method method) {
		...
		desc.setDefiningPlugin(CURRENT_PLUGIN_NAME);
