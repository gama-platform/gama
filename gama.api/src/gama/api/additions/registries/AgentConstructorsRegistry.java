 ...
 * 
 * <p>This registry maintains the mapping between agent types and their constructor implementations.
 * GAMA uses different agent implementations depending on whether the agent is a grid cell or a
 * normal agent, and whether it uses a minimal or regular implementation (with or without
 * scheduling capabilities).</p>
 * 
 * <h2>Agent Implementation Types</h2>
 * <p>GAMA provides four base agent implementation classes:</p>
 * <ul>
 ...
 * </ul>
 * 
 * <p>The appropriate base class is selected at runtime based on the species definition
 * and whether the agent belongs to a grid.</p>
 * 
 * <h2>Constructor Registration</h2>
 * <p>Agent constructors are registered during platform initialization by the core
 * plugin. Each constructor is wrapped in an {@link IAgentConstructor} functional interface
 * for efficient instantiation.</p>
 * 
 * <h2>Usage</h2>
 * <pre>{@code
...
public class AgentConstructorsRegistry {
	...
	/** The minimal grid agent implementation class. */
	static Class<? extends IAgent> minimalGrid;

	/** The regular (full-featured) grid agent implementation class. */
	 ...
	 * 
	 * <p>This method is called during platform initialization to register the base
	 * agent implementations. It stores the class reference for later retrieval and
	 * wraps the class constructor in an {@link IAgentConstructor} for efficient
	 * instantiation.</p>
	 * 
	 * <p>The method assumes the agent class has exactly one constructor that takes
	 * a population manager and an index as parameters.</p>
	 *
	 * @param clazz the agent implementation class to register
	 * @param isGrid true if this is a grid cell implementation, false for normal agents
	 * @param isMinimal true if this is a minimal (no scheduling) implementation, false for regular
	 */
	 ...
	 * 
	 * <p>This method selects one of the four registered base agent implementation classes
	 * based on whether the agent is a grid cell and whether it requires a minimal or
	 * regular (scheduled) implementation.</p>
	 * 
	 * <p>The returned class can be used to:</p>
	 * <ul>
	 ...
	 * </ul>
	 *
	 * @param isGrid true to get a grid cell implementation, false for a normal agent
	 * @param isMinimal true to get a minimal (no scheduling) implementation, false for regular
