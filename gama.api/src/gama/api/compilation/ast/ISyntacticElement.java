 ...
 *
 * <p>
 * This interface defines the core abstraction for all syntactic elements that form the AST of a GAML model. Syntactic
 * elements represent statements, symbols, and other language constructs as parsed from source code, maintaining the
 * hierarchical structure and preserving source location information.
 * </p>
 *
 * <h2>Purpose</h2>
 *
 ...
 *
 * <p>
 * Syntactic elements are generally not thread-safe and should only be accessed from a single thread during construction
 * and traversal. Once the AST is built, it is typically immutable during description building and can be safely read by
 * multiple threads.
 * </p>
 *
 * @author drogoul
...
public interface ISyntacticElement extends INamed, IDisposable {
	 ...
	 *
	 * <p>
	 * Implementations of this interface can be passed to various visit methods to process syntactic elements during AST
	 * traversal. This follows the Visitor design pattern.
	 * </p>
	 *
	 * <h3>Usage Example:</h3>
	 *
	 * <pre>{@code
	 * SyntacticVisitor visitor = element -> { System.out.println("Visiting: " + element.getKeyword()); };
	...
	public interface SyntacticVisitor {
		 ...
		 *
		 * <p>
		 * This method is called for each element encountered during traversal. Implementations can inspect, modify, or
		 * collect information about the element.
		 * </p>
		 *
		 * @param element
		 *            the syntactic element being visited (never null)
		 */
	 ...
	 *
	 * <p>
	 * This visitor calls {@link #dispose()} on each visited element, releasing resources and clearing references.
	 * Useful for cleaning up the entire AST when it's no longer needed.
	 * </p>
	 *
	 * <h3>Usage:</h3>
	 *
	 ...
	 *
	 * <p>
	 * Matches elements that are species but not grids. Can be used with stream operations or custom traversal logic to
	 * filter only regular species.
	 * </p>
	 */
	 ...
	 *
	 * <p>
	 * The keyword identifies the type of GAML construct this element represents (e.g., "species", "action", "if",
	 * "create"). This should typically be set during element construction and not changed afterwards.
	 * </p>
	 *
	 * @param name
	 *            the GAML keyword for this element (e.g., "species", "action", "reflex")
	 */
	 ...
	 *
	 * <p>
	 * Facets are key-value pairs that represent element properties/parameters. Common facets include "name", "type",
	 * "init", "size", etc.
	 * </p>
	 *
	 * <h3>Example:</h3>
	 *
	 * <pre>{@code
	 * if (element.hasFacet("name")) { String name = element.getName(); }
	 * }</pre>
	 *
	 * @param name
	 ...
	 *
	 * <p>
	 * Facets store their values as {@link IExpressionDescription} objects, which can represent constants, variables,
	 ...
	 * <pre>{@code
	 * IExpressionDescription sizeExpr = element.getExpressionAt("size");
	 * if (sizeExpr != null && sizeExpr.isConst()) { Object value = sizeExpr.getExpression().literalValue(); }
	 * }</pre>
	 *
	 * @param name
	 ...
	 *
	 * <p>
	 * This method creates a clean copy of the facets map, potentially applying transformations based on the symbol
	 * prototype. The copy is independent and modifications won't affect the original element.
	 * </p>
	 *
	 * <p>
	 ...
	 *
	 * @param sp
	 *            the symbol prototype, may be null
	 ...
	 *
	 * <p>
	 * If a facet with this name already exists, it will be replaced. Otherwise, a new facet is added.
	 * </p>
	 *
	 * <h3>Example:</h3>
	 *
	 ...
	 * @param expr
	 *            the expression description for the facet value
	 */
	 ...
	 *
	 * <p>
	 * The visitor's {@code visit} method will be called once for each facet, receiving the facet name and expression
	 * description.
	 * </p>
	 *
	 * <h3>Example:</h3>
	 ...
	 *
	 * @param visitor
	 *            the facet visitor to apply (must not be null)
	 */
	 ...
	 *
	 * <p>
	 * The name is typically extracted from the "name" facet, if present. Not all elements have names (e.g., anonymous
	 ...
	 *
	 * <p>
	 * This links the syntactic element back to the original parsed EMF model, which contains source location
	 * information and is used for error reporting. Synthetic elements (created programmatically rather than parsed) may
	 * return null.
	 * </p>
	 *
	 ...
	 *
	 * <p>
	 * This builds the hierarchical AST structure. For example, a species element would have action, variable, and
	 * reflex elements as children.
	 * </p>
	 *
	 * <p>
	 * Not all elements support children. Adding a child to an element that doesn't support them may have no effect or
	 ...
	 *
	 * @param e
	 *            the child syntactic element to add (must not be null)
	 */
	 ...
	 *
	 * <p>
	 * This is more efficient than checking if the facet count is zero, as it may avoid creating collections.
	 * </p>
	 *
	 ...
	 *
	 * <p>
	 * This method recursively traverses the element and its children, counting occurrences of each element type (by
	 * keyword or class name). The results are accumulated in the provided map.
	 * </p>
	 *
	 * <h3>Example:</h3>
	 *
	 ...
	 *
	 * @param stats
	 *            a map to be filled with element type frequencies (must not be null)
	 */
	 ...
	 *
	 * <p>
	 * This performs a depth-first traversal of the entire subtree rooted at this element, visiting every element
	 * exactly once. The visitor is called for this element first, then recursively for all children.
	 * </p>
	 *
	 * <h3>Example:</h3>
	 *
	 ...
	 *
	 * @param visitor
	 *            the visitor to apply to each element (must not be null)
	 */
	 ...
	 *
	 * <p>
	 * This should be called after the element is fully constructed and won't be modified further. Particularly useful
	 * for large ASTs to reduce memory consumption.
	 * </p>
	 */
	void compact();
	 ...
	 *
	 * <p>
	 * This is more efficient than getting the children list and checking its size, as it may avoid creating
	 * collections.
	 * </p>
	 *
	 ...
	 *
	 * <p>
	 * Unlike {@link #visitChildren(SyntacticVisitor)}, this method visits ALL children including species, grids, and
	 * experiments. It does not visit this element itself.
	 * </p>
	 *
	 * <h3>Example:</h3>
	 *
	 * <pre>{@code
	 * element.visitAllChildren(child -> { System.out.println("Direct child: " + child.getKeyword()); });
	 * }</pre>
	 *
	 * @param syntacticVisitor
	 *            the visitor to apply to all children (must not be null)
	 */
	 ...
	 *
	 * <p>
	 * Pragmas are compiler directives or metadata annotations that can be attached to GAML elements. They typically
	 * start with '@' in the source code and provide additional compilation hints or documentation.
	 * </p>
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * This method releases temporary resources or caches used during compilation without disposing the entire element.
	 * Unlike {@link #dispose()}, the element remains valid and usable after calling this method.
	 * </p>
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * The path typically represents the location of this element in the source file or the hierarchical path within the
	 * model structure. This is used for error reporting and debugging.
	 * </p>
	 *
	 * <h3>Example paths:</h3>
	 * <ul>
