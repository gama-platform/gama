/*******************************************************************************************************
 *
 * GamlRegularDocumentation.java, in gama.core, is part of the source code of the GAMA modeling and simulation platform
 * (v.2025-03).
 *
 ...
 *
 * <p>
 * This class provides a flexible, mutable implementation of {@link IGamlDocumentation} that allows dynamic building of
 * documentation content through appending and prepending operations. It also supports organizing sub-documentation into
 * categories (headers) with named entries.
 * </p>
 *
 * <h2>Purpose</h2>
 *
 ...
 * GamlRegularDocumentation actionDoc = new GamlRegularDocumentation("Moves the agent towards a target location.");
 *
 * // Add parameter documentation under "Parameters" header
 * actionDoc.set("Parameters", "speed", new GamlConstantDocumentation("Movement speed in meters per second"));
 * actionDoc.set("Parameters", "target", new GamlConstantDocumentation("Target location as a point"));
 *
 * // Add return value documentation
 * actionDoc.set("Returns", "success", new GamlConstantDocumentation("True if movement was successful"));
 *
 ...
 *
 * <p>
 * This class preserves the natural alphabetical order of headers and entries while serializing access to its mutable
 * state.
 * Concurrent reads and writes on the same instance therefore do not trigger {@link java.util.ConcurrentModificationException}s.
 * Each call to {@link #getContents()} builds its HTML from a stable snapshot of the current sub-documentation.
 * </p>
 *
 * <p>
 * The consistency of nested documentation still depends on the thread-safety of the {@link IGamlDocumentation}
 * instances stored as sub-documentation values. For immutable nested documentation, use
...
public class GamlRegularDocumentation implements IGamlDocumentation {
	...
	/** Lock protecting the builder and the alphabetically ordered sub-documentation maps. */
	private final Object lock = new Object();

	/** Map of headers to their sub-documentation entries (organized as header -> key -> doc). */
	final Map<String, Map<String, IGamlDocumentation>> subdocs = new TreeMap<>();

	/**
	 * Creates documentation with initial content.
	 *
	 * <p>
	 * The provided character sequence becomes the initial content of the documentation. Additional content can be
	 * appended or prepended afterwards.
	 * </p>
	 *
	 * @param sb
	 *            the initial documentation content
	 */
	 ...
	 *
	 * <p>
	 * The documentation starts with no content. Use {@link #append(String)} or {@link #prepend(String)} to add content.
	 * </p>
	 */
	 ...
	 *
	 * <p>
	 * The string is added to the end of the current main documentation text. This does not affect sub-documentation.
	 * </p>
	 *
	 * @param string
	 ...
	 *
	 * <p>
	 * If no sub-documentation exists, returns the main content as-is. If sub-documentation is present, generates
	 * formatted HTML with the main content followed by sections for each header containing lists of sub-documentation
	 * entries.
	 * </p>
	 *
	...
	public String getContents() {
		...
		final Map<String, Map<String, IGamlDocumentation>> snapshot;
		synchronized (lock) {
			if (subdocs.isEmpty()) return builder.toString();
			contents = builder.toString();
			snapshot = new TreeMap<>();
			subdocs.forEach((header, category) -> snapshot.put(header, new TreeMap<>(category)));
		}
		StringBuilder sb = new StringBuilder(contents);
		snapshot.forEach((header, category) -> {
			sb.append("<hr/>").append(header).append("<br/><ul>");
			category.forEach((name, doc) -> sb.append("<li><b>").append(name).append("</b>: ").append(doc));
			sb.append("</ul><br/>");
		});
		return sb.toString();
	}
	 ...
	 *
	 * <p>
	 * This method organizes sub-documentation into hierarchical sections. Each header represents a category (e.g.,
	 * "Parameters", "Returns"), and within each header, individual entries are identified by keys (e.g., parameter
	 * names).
	 * </p>
	 *
	 * <h3>Example:</h3>
	 ...
	 *
	 * @param header
	 *            the category/section header under which to organize this entry
	 * @param key
	 *            the specific identifier for this sub-documentation entry
	 * @param doc
	 *            the documentation to associate with this header and key
	 */
	@Override
	public void set(final String header, final String key, final IGamlDocumentation doc) {
		synchronized (lock) {
			subdocs.computeIfAbsent(header, h -> new TreeMap<>()).put(key, doc);
		}
	}
	 ...
	 *
	 * <p>
	 * Searches all headers for an entry with the specified key and returns the first match. The search order follows the
	 * natural alphabetical order of headers.
	 * </p>
	 *
	 * <h3>Example:</h3>
	 *
	 ...
	 *
	 * @param key
	 *            the key identifying the sub-documentation to retrieve
	...
	public IGamlDocumentation get(final String key) {
		synchronized (lock) {
			for (Map<String, IGamlDocumentation> category : subdocs.values()) {
				IGamlDocumentation doc = category.get(key);
