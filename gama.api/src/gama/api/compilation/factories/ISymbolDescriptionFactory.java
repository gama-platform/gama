 ...
 *
 * <p>
 * This interface defines the contract for factories that transform syntactic elements (parsed from source code) into
 * semantic descriptions. Symbol description factories are responsible for creating the appropriate {@link IDescription}
 * subtype based on the symbol keyword and kind.
 * </p>
 *
 * <h2>Purpose</h2>
 *
 ...
 *
 * 		&#64;Override
 * 		public int[] getKinds() { return new int[] { ISymbolKind.SPECIES }; }
 *
 * 		@Override
 * 		public ISpeciesDescription createBuiltInSpeciesDescription(String name, Class clazz,
 * 				ISpeciesDescription macro, ISpeciesDescription parent, IAgentConstructor helper, Set<String> skills,
 * 				String plugin) {
 ...
 */
public interface ISymbolDescriptionFactory {

	/**
	 * Specialized factory for creating species descriptions.
	 *
	 * <p>
	 * Species are agent type definitions in GAML. This interface adds capability for creating built-in species provided
	 * by the platform.
	 * </p>
	 */
	interface Species extends ISymbolDescriptionFactory {
		 ...
		 *
		 * <p>
		 * This method creates platform-provided species like "agent", "world", "grid", etc. These species are defined
		 * in Java code rather than GAML source.
		 * </p>
		 *
		 * <h3>Example:</h3>
		 *
		 ...
		 * ISpeciesDescription agentSpecies = factory.createBuiltInSpeciesDescription("agent", GamlAgent.class, null, // no
		 * 																											// macro
		 * 		null, // no parent (root species)
		 * 		new AgentConstructor(), Set.of("moving", "visible"), "gama.core");
		 * }</pre>
		 *
		 * @param name
		 ...
		 * @param macro
		 *            the macro species (for grid species), may be null
		 * @param parent
		 *            the parent species, may be null for root species
		 * @param helper
		 *            the agent constructor for creating instances
		 * @param skills
		 *            the set of skill names the species has
		 * @param plugin
		 *            the plugin defining this species
	 ...
	 *
	 * <p>
	 * Skills are modules that augment species with additional attributes and behaviors. This interface adds capability
	 * for creating built-in skills provided by the platform.
	 * </p>
	 */
	interface Skill extends ISymbolDescriptionFactory {
		 ...
		 *
		 * <p>
		 * This method creates platform-provided skills like "moving", "grid", "3d", etc. These skills are defined in
		 * Java code and registered with the platform.
		 * </p>
		 *
		 * <h3>Example:</h3>
		 *
		 ...
		 * ISkillDescription movingSkill =
		 * 		factory.createBuiltInSkillDescription("moving", MovingSkill.class, childrenDescriptions, // actions and
		 * 																									// variables
		 * 				"gama.core");
		 * }</pre>
		 *
		 ...
		 * @param clazz
		 *            the Java class implementing the skill
		 * @param children
		 *            the attributes and actions provided by the skill
		 * @param plugin
		 *            the plugin defining this skill
	 ...
	 *
	 * <p>
	 * This is the main factory method called during compilation to transform syntactic elements into semantic
	 * descriptions. It creates the appropriate description type based on the keyword and symbol prototype.
	 * </p>
	 *
	 * <p>
	 ...
	 * IDescription desc = factory.buildDescription("species", // keyword
	 ...
	 *
	 * @param keyword
	 *            the GAML keyword (e.g., "species", "action", "if")
	 * @param facets
	 *            the facets (properties) of the symbol
	 * @param element
	 *            the source EObject for error reporting and location tracking
	 * @param children
	 *            the child descriptions (nested elements)
	 * @param enclosing
	 *            the enclosing description (parent in the hierarchy)
	 * @param proto
	 *            the symbol prototype containing metadata about this symbol type
	 ...
	 *
	 * <p>
	 * This method identifies which symbol categories this factory handles. Factories are registered and retrieved based
	 * on these kinds.
	 * </p>
	 *
	 * <h3>Example:</h3>
	 ...
	 * <pre>{@code
	 * @Override
	 * public int[] getKinds() { return new int[] { ISymbolKind.SPECIES, ISymbolKind.GRID }; }
	 * }</pre>
	 *
	 * @return an array of symbol kind constants from {@link ISymbolKind}
	 */
	ISymbolKind[] getKinds();
