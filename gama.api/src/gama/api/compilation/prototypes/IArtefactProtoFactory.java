 ...
 * 
 * <p>
 * This interface defines the contract for factories that create {@link IArtefactProto} instances
 * during platform initialization. Prototypes are constructed from Java annotations on classes and
 * methods that define GAML symbols and operators.
 * </p>
 * 
 * <h2>Purpose</h2>
 * 
 ...
 * </p>
 * <ul>
 *   <li><strong>Annotation Processing:</strong> Converting {@code @symbol} and {@code @operator} annotations to prototypes</li>
...
public interface IArtefactProtoFactory {
	 ...
	 * 
	 * <p>
	 * This method constructs a complete symbol prototype containing all structural and
	 * behavioral information needed for compilation and validation.
	 * </p>
	 *
	 * @param c the Java class implementing the symbol
	 * @param isBreakable whether the symbol can contain break statements
	 * @param isContinuable whether the symbol can contain continue statements
	 * @param isSequence whether the symbol is a sequence (contains sub-statements)
	 * @param hasArguments whether the symbol has formal arguments
	 * @param sKind the symbol kind (from {@code ISymbolKind})
	 * @param isPrimitive whether this is a primitive (built-in) symbol
	 * @param fmd array of facet prototypes describing allowed facets
	 * @param omissible the name of the facet that can be omitted, or null
	 * @param contextKeywords keywords of contexts where this symbol can appear
	 * @param contextKinds kinds of contexts where this symbol can appear
	 * @param isRemoteContext whether expressions in this symbol are evaluated remotely
	 * @param isUnique whether only one instance can exist in the model
	 * @param name_unique whether the name must be unique
	 * @param sc the symbol factory for creating instances
	 * @param name the symbol name/keyword
	 * @param plugin the plugin defining this symbol
	...
	IArtefactProto.Symbol createSymbolProto(Class c, boolean isBreakable, boolean isContinuable, boolean isSequence,
			boolean hasArguments, int sKind, boolean isPrimitive, IArtefactProto.Facet[] fmd, String omissible,
	 ...
	 * 
	 * <p>
	 * This overload is used when the operator signature is specified as a Class object
	 * rather than a full {@code Signature} instance.
	 * </p>
	 *
	 * @param name the operator name
	 * @param object the annotated element (method or class)
	 * @param helper the helper for executing the operator
	 * @param canBeConst whether the operator can be constant-folded
	 * @param isIterator whether the operator is an iterator
	 * @param returnType the return type code
	 * @param signature the signature as a Class
	 * @param typeProvider type provider code for return type
	 * @param contentTypeProvider type provider for content type
	 * @param keyTypeProvider type provider for key type
	 * @param expectedContentType expected content types array
	 ...
	 * 
	 * <p>
	 * This is the complete factory method providing all operator metadata including
	 * documentation and a full type signature.
	 * </p>
	 *
	 * @param name the operator name
	 * @param method the Java method implementing the operator
	 * @param constantDoc constant documentation string
	 * @param helper the helper for executing the operator
	 * @param canBeConst whether the operator can be constant-folded
	 * @param isIterator whether the operator is an iterator
	 * @param rt the return type
	 * @param signature the full signature specifying parameter types
	 * @param typeProvider type provider code for return type
	 * @param contentTypeProvider type provider for content type
	 * @param keyTypeProvider type provider for key type
	 * @param contentContentTypeProvider type provider for content's content type
	 * @param expectedContentTypes expected content types array
	 * @param plugin the plugin defining this operator
	 * @return a new operator prototype
	 */
	 ...
	 * 
	 * <p>
	 * Facet prototypes define the allowed properties that can be attached to symbols,
	 * including their types, allowed values, and behavioral flags.
	 * </p>
	 *
	 * @param name the facet name
	 * @param types array of type codes for allowed expression types
	 * @param contentType content type code (for container types)
	 * @param keyType key type code (for map types)
	 * @param values restricted set of allowed literal values, or null
	 * @param optional whether the facet is optional
	 * @param internal whether the facet is internal (not user-facing)
	 * @param isRemote whether expressions are evaluated in remote context
