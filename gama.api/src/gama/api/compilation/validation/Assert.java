 ...
 * 
 * <p>This class serves as a centralized location for validation logic used throughout the GAML compilation
 * process. It provides static utility methods to verify:</p>
 * 
 * <ul>
 *   <li><strong>Type Compatibility:</strong> Ensuring assigned values can be safely converted to receiver types</li>
 *   <li><strong>Name Validity:</strong> Checking that identifiers don't conflict with reserved keywords or type names</li>
 ...
 * 
 * <p>The class handles several special cases in the GAML type system:</p>
 * <ul>
 ...
 * 
 * <p>The following keywords are reserved and cannot be used as variable or entity names:</p>
 * <ul>
 ...
 * Assert.typesAreCompatibleForAssignment(
 *     "value",                  // facet name
 ...
 * 
 * <p>All validation methods report issues directly to the description context using appropriate
 * issue codes from {@link IGamlIssue}:</p>
 * <ul>
...
public class Assert implements gama.api.constants.IKeyword {
	 ...
	 * 
	 * <p>This method performs comprehensive type checking for assignments, ensuring that the assigned
	 * expression can be safely converted to the receiver type. It handles multiple special cases
	 * in the GAML type system and emits appropriate warnings or errors.</p>
	 * 
	 ...
	 *   <li><strong>Species Incompatibility:</strong> Validates sub-species relationships (see {@link #speciesAreIncompatible})</li>
	 ...
	 * </ul>
	 *
	 * @param facetName the name of the facet being assigned (e.g., "value", "init")
	 * @param context the description context where the assignment occurs (for error reporting)
	 * @param receiverDescription a human-readable description of what receives the value (e.g., "variable x")
	 * @param receiverType the expected type of the receiver
	 * @param assigned the expression description being assigned (may be null)
	 * 
	 ...
	 * 
	 * <p>This method handles two special cases in the GAML type system:</p>
	 * 
	 * <h3>1. Int/Float Conversion (Issue #590):</h3>
	 * <p>When an integer value is assigned to a float variable (or vice versa), GAML performs
	 * an automatic cast but emits a warning to alert the developer of the potential precision
	 * loss or conversion.</p>
	 * 
	 * <h3>2. Empty Container Assignment:</h3>
	 * <p>Empty lists and maps (e.g., {@code []}, {@code map([])}) are accepted for any container
	 * type assignment, as they don't carry type-specific content that could cause issues.</p>
	 * 
	 * <h3>Warning Generated:</h3>
	 * <p>When a conversion is needed, the method emits a {@link IGamlIssue#SHOULD_CAST} warning
	 * with the message: "{receiver} of type {receiverType} is assigned a value of type {assignedType},
	 * which will be casted to {receiverType}"</p>
	 *
	 * @param facetName the name of the facet being assigned
	 * @param context the description context for error reporting
	 * @param receiverDescription human-readable description of the receiver
	 * @param receiverType the expected type
	 * @param assigned the expression description being assigned
	 * @param value the actual expression value
	 * @param assignedType the type of the assigned value
	 * 
	 ...
	 * 
	 * <p>This method generates a {@link IGamlIssue#SHOULD_CAST} warning when the content type
	 * of a container being assigned doesn't match the expected content type. For example,
	 * assigning a {@code list<int>} to a {@code list<float>} variable.</p>
	 * 
	 * <p>The warning message indicates which elements will be automatically cast and helps
	 * developers understand potential type conversions happening in their code.</p>
	 * 
	 * <h3>Warning Format:</h3>
	 * <p>"Elements of {receiver} are of type {receiverContentType} but are assigned elements
	 * of type {contentType}, which will be casted to {receiverContentType}"</p>
	 *
	 * @param facetName the facet name being assigned
	 * @param context the description context for error reporting
	 * @param receiverDescription human-readable description of the receiver
	 * @param receiverType the full receiver type (including content type)
	 * @param assigned the expression description being assigned
	 * @param receiverContentType the expected content type of the container
	 * @param contentType the actual content type being assigned
	 */
	 ...
	 * 
	 * <p>This method validates species hierarchy constraints when assigning species values.
	 * In GAML, you can only assign a species to a variable of type {@code species<X>} if the
	 * assigned species is X itself or a sub-species of X.</p>
	 * 
	 * <h3>Validation Rule:</h3>
	 * <p>For assignments to {@code species<Parent>} variables:</p>
	 * <ul>
	 ...
	 * <p>When the assigned species is not a sub-species of the receiver species, emits error:
	 * "Impossible assignment: {assignedSpecies} is not a sub-species of {receiverSpecies}"
	 * with issue code {@link IGamlIssue#WRONG_TYPE}.</p>
	 *
	 * @param context the description context for error reporting
	 * @param receiverType the full receiver type (must be {@code species<X>})
	 * @param assigned the expression description being assigned
	 * @param expr2 the actual expression value
	 * @param receiverContentType the expected species type (X in {@code species<X>})
	 * @param contentType the assigned species type
	 ...
	 * 
	 * <p>These keywords have special meaning in the GAML language and are reserved for
	 * language constructs. Attempting to use them as variable names, action names, species
	 * names, etc., will result in a compilation error.</p>
	 * 
	 ...
	 * 
	 * <p>This method performs three critical validations on identifier names:</p>
	 * 
	 ...
	 * <p>Error messages adapt based on the description type:</p>
	 * <ul>
	 ...
	 * }</pre>
	 *
	 * @param cd the description to validate (must have a name attribute)
	...
	public static boolean nameIsValid(final IDescription cd) {
		...
		final IModelDescription md = cd.getModelDescription();
