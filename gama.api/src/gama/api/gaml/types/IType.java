...
/**
 * The central interface defining the type system in GAMA. All GAML types
 * (built-in, species, parametric containers,
 ...
 * <li>Type hierarchy and relationships (assignability, distance, common
 * supertypes)</li>
 ...
 *
 * <p>
 * Each type has a unique integer ID, a name (used in GAML code), and a Java
 * support class that represents values of
 ...
 *
 * @param <Support>
 *                  the Java class representing values of this type at runtime
 *                  (e.g., Integer for int, GamaPoint for point)
 *
 * @author drogoul
 * @since GAMA 1.0
 *
...
public interface IType<Support> extends IGamlDescription, ITyped, IJsonable {
	...
	/**
	 * Array of vowels used for article determination ("a" vs "an") in error
	 * messages.
	 */
	...
	/**
	 * Starting ID for custom user-defined types. IDs >= this value are for custom
	 * types.
	 */
	...
	/**
	 * Starting ID for file types. IDs >= this value represent specific file
	 * formats.
	 */
	...
	/**
	 * Casts an object to this type's support class. This is the fundamental
	 * conversion operation in GAMA's type system.
	 *
	 * <p>
	 * Examples:
	 * </p>
	 ...
	 *
	 * @param scope
	 *              the current execution scope, provides access to agents,
	 *              simulation state, etc.
	 * @param obj
	 *              the object to cast (can be any Java object)
	 * @param param
	 *              optional parameter for the casting operation (type-specific, can
	 *              be null)
	 * @param copy
	 *              if true, creates a copy of the object rather than reusing it
	 * @return the object cast to this type's support class, or the default value if
	 *         casting fails
	 *
	 * @see #getDefault()
	 */
	...
	/**
	 * Casts an object to this type with explicit key and content types (for
	 * parametric container types).
	 *
	 * <p>
	 * This method is primarily used for container types (list, map, matrix, etc.)
	 * that have parametric content.
	 * </p>
	 *
	 * <p>
	 * Examples:
	 ...
	 * @param keyType
	 *                    the type of keys (for maps) or indices (for other
	 *                    containers)
	 * @param contentType
	 *                    the type of content/values in the container
	 * @param copy
	 *                    if true, creates a copy of the object
	 * @return the object cast to this type's support class with specified
	 *         parametric types
	 */
	 ...
	 *
	 * <p>
	 * Type IDs are used for efficient type comparison and switching. Built-in types
	 * have fixed IDs (see constants
	 * above), while dynamic types (species, custom types) have IDs allocated at
	 * runtime.
	 * </p>
	 *
	 * @return the type ID
	...
	/**
	 * Returns the default value for this type. Used when initializing variables
	...
	/**
	 * Returns the variable kind constant for this type, used in GAML grammar and
	 * variable declarations.
	 *
	 * <p>
	 * Variable kinds distinguish between different syntactic categories (regular
	 ...
	 *
	 * <p>
	 * Fields represent attributes and pseudo-attributes that can be accessed using
	 * the dot notation in GAML (e.g.,
	 ...
	 *
	 * <p>
	 * The map keys are field names, and values are the corresponding operator
	 * prototypes.
	 * </p>
	 *
	 ...
	 *
	 * <p>
	 * For example, if this type represents a species defined as
	 * {@code species mySpecies {...}}, this returns
	 * "mySpecies".
	 * </p>
	 *
	 ...
	 *
	 * <p>
	 * The species description contains all metadata about the species: its
	 * attributes, actions, parent species, etc.
	 * </p>
	 *
	...
	/**
	 * Returns the species description denoted by this type (for species container
	 * types).
	 *
	 * <p>
	 * This is used for types that represent containers of agents of a specific
	 * species, such as when using species as a
	...
	/**
	 * Checks if a value of type {@code l} can be assigned to a variable of this
	 * type without explicit casting.
	 *
	 * <p>
	 * This is the fundamental assignability check in GAMA's type system. The
	 * relation is transitive: if A is assignable
	 ...
	 * </p>
	 * <ul>
	 * <li>Types.FLOAT.isAssignableFrom(Types.INT) returns true (ints can be
	 * assigned to floats)</li>
	 * <li>Types.INT.isAssignableFrom(Types.FLOAT) returns false (floats cannot be
	 * assigned to ints without
	 * casting)</li>
	 * <li>Types.GEOMETRY.isAssignableFrom(Types.POINT) returns true (points are
	 * geometries)</li>
	 * </ul>
	 *
	 * @param l
	...
	/**
	 * Internal computation of isAssignableFrom without cache lookup. Used by
	 * TypesManager to avoid infinite recursion.
	 *
	 * <p>
	 * Implementations should provide the actual assignability logic without
	 * delegating to the cache. This method is
	 * called by {@link ITypesManager#checkAssignability(IType, IType)} which
	 * manages caching.
	 * </p>
	 *
	 * @param l
	...
	/**
	 * Checks if values of this type can be translated (converted with information
	 * loss) into the target type.
	 *
	 * <p>
	 * Translation is more permissive than assignment. For example, float can be
	 * translated into int (with truncation),
	 ...
	 * <li>Types.POINT.isTranslatableInto(Types.LIST) returns true (extraction of
	 * coordinates)</li>
	 * </ul>
	 *
	 * @param t
	...
	/**
	 * Internal computation of isTranslatableInto without cache lookup. Used by
	 * TypesManager to avoid infinite
	 * recursion.
	 *
	 * <p>
	 * Implementations should provide the actual translatability logic without
	 * delegating to the cache. This method is
	 * called by {@link ITypesManager#checkTranslatability(IType, IType)} which
	 * manages caching.
	 * </p>
	 *
	 * @param t
	 ...
	 *
	 * <p>
	 * The type hierarchy forms a tree rooted at Types.NO_TYPE. Parent-child
	 * relationships are based on Java class
	 ...
	 *
	 * <p>
	 * All types except NO_TYPE have a parent. This is used for inheritance of
	 * fields and type compatibility checks.
	 * </p>
	 *
	...
	/**
	 * Coerces an expression's type to make it compatible with this type in a given
	 * context.
	 *
	 * <p>
	 * This method is used during compilation to determine if an expression needs
	 * type conversion. It may return a
	 * different type than this or the expression type, representing the most
	...
	/**
	 * Computes the distance between this type and another type in the type
	 * hierarchy.
	 *
	 * <p>
	 * The distance represents the number of steps in the type hierarchy between two
	 ...
	 * <li>Types.STRING.distanceTo(Types.INT) returns Integer.MAX_VALUE (no
	 * relationship)</li>
	 * </ul>
	 *
	 * @param originalChildType
	 *                          the target type to compute distance to
	 * @return the distance (0 = same type, positive = steps in hierarchy, MAX_VALUE
	 *         = unreachable)
	 *
	...
	/**
	 * Internal computation of distanceTo without cache lookup. Used by TypesManager
	 * to avoid infinite recursion.
	 *
	 * <p>
	 * Implementations should provide the actual distance computation logic without
	 * delegating to the cache. This method
	 * is called by {@link ITypesManager#computeDistance(IType, IType)} which
	 * manages caching.
	 * </p>
	 *
	 * @param originalChildType
	 ...
	 *
	 * <p>
	 * The common supertype is the most specific type that both types can be
	 * assigned to. This is used for type
	 ...
	 * <li>Types.POINT.findCommonSupertypeWith(Types.GEOMETRY) returns
	 * Types.GEOMETRY</li>
	 * <li>Types.STRING.findCommonSupertypeWith(Types.INT) returns
	 * Types.NO_TYPE</li>
	 * </ul>
	 *
	 * @param iType
	...
	/**
	 * Internal computation of findCommonSupertypeWith without cache lookup. Used by
	 * TypesManager to avoid infinite
	 * recursion.
	 *
	 * <p>
	 * Implementations should provide the actual common supertype logic without
	 * delegating to the cache. This method is
	 * called by {@link ITypesManager#computeCommonSupertype(IType, IType)} which
	 * manages caching.
	 * </p>
	 *
	 * @param iType
	...
	/**
	 * Determines the actual type that results from casting an expression to this
	 * type.
	 *
	 * <p>
	 * Usually returns this type, but some types (like species types or agent types)
	 * compute a more specific type based
	 * on the expression being cast.
	 * </p>
	 *
	 * <p>
	 * For example, casting an agent to a species type might return the specific
	 * species type of that agent.
	 * </p>
	 *
	 * @param exp
	 ...
	 *
	 * <p>
	 * Container types hold collections of values and support iteration, indexing,
	 * and other container operations.
	 * </p>
	 *
	 ...
	 *
	 * <p>
	 * Types like matrices, strings, and points have fixed length. Lists and maps do
	 * not.
	 * </p>
	 *
	 * <p>
	 ...
	 * </ul>
	 *
	 * @return true if the type has fixed length and cannot be modified via
	 *         add/remove
	 */
	 ...
	 *
	 * <p>
	 * For non-container types, this may return the type of components (e.g., float
	 * for point).
	 * </p>
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * For maps, this is the actual key type. For lists and matrices, this is the
	 * index type (int).
	 * </p>
	 *
	 * <p>
	...
	/**
	 * Checks if this type represents a compound value which components can be
	 * extracted when casting to a container.
	 *
	 * <p>
	 * For instance, points have float components that can be extracted as a list.
	 * Containers are compound types by
	 * default.
	 * </p>
	 *
	 * <p>
	...
	/**
	 * Returns the wrapped type for wrapper types (like file types wrapping specific
	 * formats).
	 *
	 * <p>
	 * For most types, returns Types.NO_TYPE. File types return their buffer type
	 * (csv, shapefile, etc.).
	 * </p>
	 *
	...
	/**
	 * Checks if an object can be considered an instance of this type in the given
	 * scope.
	 *
	 * <p>
	 * This is a runtime check that may involve scope-dependent logic (e.g.,
	 * checking if an agent belongs to a specific
	 * species).
	 * </p>
	 *
	 * @param s
	 ...
	 *
	 * <p>
	 * Most primitive types (int, float, bool, string) can be cast to const. Complex
	 * types (agents, files) typically
	 * cannot.
	 * </p>
	 *
	...
	/**
	 * Checks if values of this type are comparable (implement Comparable
	 * interface).
	 *
	 * <p>
	 * Comparable types can be used in comparisons (<, >, <=, >=) and sorting
	 * operations.
	 * </p>
	 *
	...
	/**
	 * Returns a string pattern representation of this type for use in regular
	 ...
	 *
	 * <p>
	 * Used for documentation and error messages to indicate which GAMA extension
	 * provides this type.
	 * </p>
	 *
	 * @param plugin
	 ...
	 *
	 * <p>
	 * This method is called during documentation generation to include
	 * type-specific field information.
	 * </p>
	 *
	 * @param result
	 ...
	 *
	 * <p>
	 * The default implementation retrieves text from the clipboard and casts it to
	 * this type.
	 * </p>
	 *
	 * @param scope
	 *              the current execution scope
	 * @return a value of this type parsed from the clipboard, or the default value
	 *         if parsing fails
	 *
	...
	/**
	 * Returns the expression associated with this type (for types defined by
	 * expressions).
	 *
	 * <p>
	 * Some types are defined dynamically based on expressions (e.g., species types
	 * from species expressions).
	 * </p>
	 *
	...
	/**
	 * Returns the types manager that owns this type and provides cached type
	 * relation operations.
	 *
	 * <p>
	 * The types manager handles caching of expensive type relationship computations
	 * (assignability, distance, etc.).
	 * </p>
	 *
