/*******************************************************************************************************
 *
 * JavaUtils.java, in gama.api, is part of the source code of the GAMA modeling and simulation platform (v.2025-03).
 *
...
public class JavaUtils {
	...
	private static int keyOf(final Class base, final Iterable<Class<? extends ISkill>> others) {
		// We use the canonical name to avoid issues with class loaders
		int result = base.getCanonicalName().hashCode();
		for (final Class other : others) { result += other.getCanonicalName().hashCode(); }
		return result;
	}
