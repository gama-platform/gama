...
import static gama.api.utils.server.ISocketCommand.PARAMETERS;
import static gama.api.utils.server.ISocketCommand.SYNC;
import static gama.api.utils.server.ISocketCommand.SYNTAX;
import static gama.api.utils.server.MessageType.CommandExecutedSuccessfully;
import static gama.api.utils.server.MessageType.MalformedRequest;
import static gama.api.utils.server.MessageType.UnableToExecuteRequest;
	...
	public static GamaServerMessage EVAL(final IGamaServer server, final WebSocket socket,
		...
		if (entered.startsWith("?")) {
			...
			try {
				...
				if (expression != null) {
					res = GAMA.getJsonEncoder().valueOf(scope.evaluate(expression, agent).getValue()).toString();
				}
