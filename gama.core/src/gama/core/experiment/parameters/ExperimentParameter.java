...
public class ExperimentParameter extends Symbol implements IParameter.Batch {
	...
	public static class ExperimentParameterValidator extends AttributeDeclaration.VarValidator {
		...
		public void validate(final IDescription vd) {
			...
			final IVariableDescription targetedGlobalVar = findTargetedVar(vd, varName);
			final IExperimentDescription ed = (IExperimentDescription) vd.getEnclosingDescription();
	 ...
	 */
	public static IVariableDescription findTargetedVar(final IDescription parameterDescription, final String varName) {
		// We look first in the model to make sure that built-in parameters (like seed) are correctly retrieved
		final IModelDescription wd = parameterDescription.getModelDescription();
		IVariableDescription targetedGlobalVar = wd.getAttribute(varName);
		if (targetedGlobalVar == null) {
			final IExperimentDescription ed = (IExperimentDescription) parameterDescription.getEnclosingDescription();
	...
	public ExperimentParameter(final IScope scope, final IParameter p, final String title, final String category,
		...
		if (among != null) {
			this.among = GAML.getExpressionFactory().createConst(among);
		} else {
			this.among = null;
		}
		Object minValue = p.getMinValue(scope);
		if (minValue != null) {
			this.min = GAML.getExpressionFactory().createConst(minValue);
		} else {
			min = null;
		}
		Object maxValue = p.getMaxValue(scope);
		if (maxValue != null) {
			this.max = GAML.getExpressionFactory().createConst(maxValue);
		} else {
			max = null;
		}
		Object stepValue = p.getStepValue(scope);
		if (stepValue != null) {
			this.step = GAML.getExpressionFactory().createConst(stepValue);
		} else {
			step = null;
		}
		onChange = null;
	...
	public Set<Object> neighborValues(final IScope scope) throws GamaRuntimeException {
		Set<Object> neighborValues = new LinkedHashSet<>();
		try {
			if (scope != null) { scope.push(this); }
