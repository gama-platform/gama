...
@facets(value = {
	...
	@facet(name = IKeyword.TITLE, type = IType.STRING, optional = false, doc = @doc(""), internal = true),
	@facet(name = IKeyword.BENCHMARK, type = IType.BOOL, optional = true, doc = @doc("If true, make GAMA record the number of invocations and running time of the statements and operators of the simulations launched in this experiment. The results are automatically saved in a csv file in a folder called 'benchmarks' when the experiment is closed"), internal = false),
	...
	@facet(name = IKeyword.SKILLS, type = IType.LIST, optional = true, doc = @doc("the skills attached to the experiment"), internal = true),
	@facet(name = IKeyword.CONTROL, type = IType.ID, optional = true, doc = @doc("the control architecture used for defining the behavior of the experiment"), internal = true),
	@facet(name = IKeyword.KEEP_SEED, type = IType.BOOL, optional = true, doc = @doc("Allows to keep the same seed between simulations. Mainly useful for batch experiments")),
	...
	@facet(name = IKeyword.TYPE, type = IType.LABEL, optional = false, doc = @doc("The type of the experiment: `gui`, `batch`, `test`, etc.")),
	@facet(name = IKeyword.VIRTUAL, type = IType.BOOL, optional = true, doc = @doc("Whether the experiment is virtual (cannot be instantiated, but only used as a parent, false by default)")),
	@facet(name = IKeyword.SCHEDULES, type = IType.CONTAINER, of = IType.AGENT, optional = true, internal = true, doc = @doc("A container of agents (a species, a dynamic list, or a combination of species and containers) , which represents which agents will be actually scheduled when the population is scheduled for execution. For instance, 'species a schedules: (10 among a)' will result in a population that schedules only 10 of its own agents every cycle. 'species b schedules: []' will prevent the agents of 'b' to be scheduled. Note that the scope of agents covered here can be larger than the population, which allows to build complex scheduling controls; for instance, defining 'global schedules: [] {...} species b schedules: []; species c schedules: b + world; ' allows to simulate a model where the agents of b are scheduled first, followed by the world, without even having to create an instance of c.")),
	@facet(name = IKeyword.RECORD, type = {
	...
	@facet(name = IKeyword.AUTORUN, type = IType.BOOL, optional = true, doc = @doc("Whether this experiment should be run automatically when launched (false by default)")) }, omissible = IKeyword.NAME)
...
public class ExperimentPlan extends GamlSpecies implements IExperimentPlan {
	...
	public void validate(final IDescription desc) {
		...
		if (tmpDesc.hasFacet(Exploration.METHODS)) {
			...
			if (!tmpDesc.hasFacet(Exploration.SAMPLE_SIZE)) {
			    ...
			    /*
			     * System.out.println(sample); if(!((sample &
			     * (sample-1)) ==0)) { tmpDesc.
			...
			break;
		    default:
			tmpDesc.error("The sampling " + tmpDesc.getLitteral(Exploration.METHODS) + " doesn't exist yet",
				IGamlIssue.MISSING_FACET);
		    }
		}
    ...
    @Override
    public void setStopCondition(final IExpression expression) {
	...
	final List<Map<String, Object>> params = seed == null ? Collections.EMPTY_LIST
	...
	agent.getParameterValues().forEach((n, v) -> {
	    if (hasVar(n)) {
		agent.setDirectVarValue(myScope, n, v);
	    }
	});
	myScope.push(agent);
	prepareGui();
    ...
    @Override
    public void setConcurrency(final IExpression exp) {
	concurrency = exp;
    }
