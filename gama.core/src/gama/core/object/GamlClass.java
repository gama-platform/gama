...
import gaml.compiler.gaml.descriptions.ClassDescription;
...
public class GamlClass extends Symbol implements IClass {
	...
	protected final Map<String, IStatement.Action> actions = GamaMapFactory.createOrdered();
	 ...
	 */
	@Override
	public IJsonObject serializeToJson(final IJson json) {
	...
	public void dispose() {
		...
		for (IStatement.Action action : actions.values()) { action.dispose(); }
		variables.clear();
		actions.clear();
		macroSpecies = null;
		parentClass = null;
	 ...
	 */
	@Override
	public IClassDescription getDescription() { return (ClassDescription) description; }
	...
	/**
	 * Sets the parent species.
	 *
	 * @param model
	 *            the new parent species
	 */
	@Override
	public void setMacroSpecies(final IModelSpecies model) { macroSpecies = model; }
