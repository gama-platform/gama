...
@facets(value = {
	@facet(name = IKeyword.VIRTUAL, type = IType.BOOL, optional = true, doc = @doc("Declaring a display as virtual makes it invisible on screen, and only usable for display inheritance")),
	@facet(name = IKeyword.PARENT, type = IType.ID, optional = true, doc = @doc("Declares that this display inherits its layers and attributes from the parent display named as the argument. Expects the identifier of the parent display or a string if the name of the parent contains spaces")),
	@facet(name = IKeyword.BACKGROUND, type = IType.COLOR, optional = true, doc = @doc("Allows to fill the background of the display and its toolbar with a specific color. Beware that this color, used in the UI, will not be affected by the light used in the display.")),
	...
	@facet(name = "antialias", type = IType.BOOL, optional = true, doc = @doc("Indicates whether to use advanced antialiasing for the display or not. The default value is the one indicated in the preferences of GAMA ('false' is its factory default). Antialising produces smoother outputs, but comes with a cost in terms of speed and memory used. ")),
	...
	@facet(name = IKeyword.TOOLBAR, type = { IType.BOOL,
		IType.COLOR }, optional = true, doc = @doc("Indicates whether the top toolbar of the display view should be initially visible or not. If a color is passed, then the background of the toolbar takes this color")),
	@facet(name = IKeyword.FULLSCREEN, type = { IType.BOOL,
	...
	@facet(name = "show_fps", internal = true, type = IType.BOOL, optional = true, doc = @doc("Allows to enable/disable the drawing of the number of frames per second")),
	@facet(name = "axes", type = IType.BOOL, optional = true, doc = @doc("Allows to enable/disable the drawing of the world shape and the ordinate axes. Default can be configured in Preferences")),
	...
	/// LIGHT FACETS
	@facet(name = IKeyword.IS_LIGHT_ON, type = IType.BOOL, optional = true, doc = @doc("Allows to enable/disable the light at once. Default is true")),
	@facet(name = "locked", type = IType.BOOL, optional = true, doc = @doc("Allows to lock/unlock a 2D display when it opens. For 3D displays please use the `camera` statement")),
	@facet(name = "draw_diffuse_light", type = IType.BOOL, optional = true, doc = @doc(deprecated = "Define it in the various 'light' statements instead with 'show: true/false'", value = "Allows to show/hide a representation of the lights. Default is false.")),

	/// CAMERA FACETS
	@facet(name = IKeyword.CAMERA, type = IType.STRING, optional = true, doc = @doc("""
	...
	@facet(name = IKeyword.KEYSTONE, type = IType.CONTAINER, optional = true, doc = @doc("Set the position of the 4 corners of your screen ([topLeft,topRight,botLeft,botRight]), in (x,y) coordinate ( the (0,0) position is the top left corner, while the (1,1) position is the bottom right corner). The default value is : [{0,0},{1,0},{0,1},{1,1}]")),
	@facet(name = "z_near", type = IType.FLOAT, optional = true, doc = @doc("Set the distances to the near depth clipping planes. Must be positive.")),
	@facet(name = "z_far", type = IType.FLOAT, optional = true, doc = @doc("Set the distances to the far depth clipping planes. Must be positive.")),
	@facet(name = IKeyword.AUTOSAVE, type = { IType.BOOL, IType.POINT,
		IType.STRING }, optional = true, doc = @doc("""
			...
			(if it is not dynamically built, the previous file will be erased). Note that setting autosave to true in a display will synchronize all the displays defined in the experiment""")), }, omissible = IKeyword.NAME)
@inside(symbols = { IKeyword.OUTPUT, IKeyword.PERMANENT })
@validator(DisplayValidator.class)
@serializer(DisplaySerializer.class)
@doc(value = "A display refers to an independent and mobile part of the interface that can display species, images, texts or charts.", usages = {
	...
	@usage(value = "Each display can include different layers (like in a GIS).", examples = {
		@example(value = "display gridWithElevationTriangulated type: opengl {", isExecutable = false),
...
public class LayeredDisplayOutput extends AbstractOutput {
	...
	if (hasFacet(IKeyword.TYPE)) {
	    getData().setDisplayType(getLiteral(IKeyword.TYPE));
	}
	...
	if (getData().isWeb()) {
	    return IGui.GL_LAYER_VIEW_ID3;
	}
	if (getData().isOpenGL2()) {
	...
	if (e == null || e == IExpressionFactory.FALSE_EXPR) {
	    return false;
	}
	return true;
    }
	...
	return (IGraphicsScope) super.getScope();
    }
