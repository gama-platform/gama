...
public class GAMA {
	...
	if (newExperiment == null) { // DEBUG.OUT("No experiment " + id + " in
				     // model " + model.getFilePath());
	    return;
	}
	...
	if (currentExperiment == null) {
	    throw GamaRuntimeException.error("Experiment " + expName + " does not exist. Please check its name.",
		    getRuntimeScope());
	}
    ...
    /**
     * Gets the current experiment agent. Safe to use even if the experiment
     * plan is null
     *
    ...
    /**
     * Report Error: tries to report (on the UI) and returns true if the
	...
	final boolean shouldStop = (warning && GamaPreferences.Runtime.CORE_WARNINGS.getValue()
		|| !warning && shouldStopSimulation) && GamaPreferences.Runtime.CORE_REVEAL_AND_STOP.getValue();
	...
	for (final IExperimentController controller : controllers) {
	    if (!controller.processStartPause(andWait)) {
		return false;
	    }
	}
	return true;
	...
	for (final IExperimentController controller : controllers) {
	    if (!controller.processStep(andWait)) {
		return false;
	    }
	}
	return true;
	...
	for (final IExperimentController controller : controllers) {
	    if (!controller.processBack(andWait)) {
		return false;
	    }
	}
	return true;
	...
	for (final IExperimentController controller : controllers) {
	    if (!controller.processPause(andWait)) {
		return false;
	    }
	}
	return true;
	...
	for (final IExperimentController controller : controllers) {
	    if (!controller.processStart(andWait)) {
		return false;
	    }
	}
	return true;
	...
	if (!topLevelAgentListeners.contains(listener)) {
	    topLevelAgentListeners.add(listener);
	}
    }
	...
	if (currentTopLevelAgent == current && !force) {
	    return;
	}
	currentTopLevelAgent = current;
	for (ITopLevelAgentChangeListener listener : topLevelAgentListeners) {
	    listener.topLevelAgentChanged(current);
	}
    }
	...
	if (symbol instanceof IBenchmarkable ib) {
	    return benchmarkAgent.record(scope, ib);
	}
	if (symbol instanceof ISymbol is) {
	    return benchmarkAgent.record(scope, is.getDescription());
	}
	return StopWatch.NULL;
    }
	...
	if (!experimentStateListeners.contains(listener)) {
	    experimentStateListeners.add(listener);
	}
    }
	...
	for (IExperimentStateListener listener : experimentStateListeners) {
	    listener.updateStateTo(exp, state);
	}
