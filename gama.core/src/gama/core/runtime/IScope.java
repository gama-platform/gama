 ...
 * IScope instance maintains the state of execution, manages access to agents and variables, handles control flow, and
 * provides utilities for simulation operations.
 *
 * <p>
 * A scope is hierarchical in nature and typically associated with an agent. It maintains various stacks (agents,
 * symbols, attributes) that define the execution context. Scopes can be copied to create nested contexts while
 * preserving the parent context.
 * </p>
 *
 * <p>
 ...
 *
 * <p>
 * IScope is a central element in GAMA's architecture, acting as the bridge between the model specification (in GAML)
 * and its execution in the platform. Every statement execution, expression evaluation, or agent action occurs within a
 * scope that provides the complete context needed for execution.
 * </p>
 */
...
public interface IScope extends Closeable, IBenchmarkable {
	...
	/**
	 * Predefined set of flow statuses that interrupt the normal sequential execution flow. These statuses signal
	 * special execution paths like loop breaks, returns from actions, or agent termination.
	 */
	...
	/**
	 * Removes all contextual information from the scope, effectively resetting its state. This includes clearing agent
	 * stacks, variables, symbols, and other temporary data.
	 */
	void clear();

	/**
	 * Closes this scope, releasing any resources it holds. The default implementation simply calls {@link #clear()}.
	 */
	@Override
	default void close() {
		clear();
	 ...
	 *
	 * @param b
	 *            true to put the scope on hold, false to release it
	 */
	...
	/**
	 * Disables error reporting during execution, causing exceptions to be suppressed. This is useful for operations
	 * that may generate controlled or expected errors.
	 */
	void disableErrorReporting();

	/**
	 * Enables error reporting during execution, allowing exceptions to be raised and handled. This is the default
	...
	/**
	 * Enables or disables execution tracing. When tracing is enabled, the scope will output information about each
	 * execution step for debugging purposes.
	 *
	 * @param trace
	...
	/**
	 * Returns the name of this scope, which is typically used for identification and debugging purposes.
	 *
	...
	/**
	 * Returns the name to use for benchmarking purposes. The default implementation returns the same value as
	 * {@link #getName()}.
	 *
	...
	/**
	 * Creates a new scope that is a copy of this one, with an additional name component. The copy shares the same
	 * context but can have its own local variables and state.
	 *
	 * @param additionalName
	 *            Name to append to the current scope name
	...
	/**
	 * Creates a new graphics-enabled scope that is a copy of this one, with an additional name. The resulting scope
	 * supports graphical operations through the IGraphicsScope interface.
	 *
	 * @param additionalName
	 *            Name to append to the current scope name
	 * @return A new IGraphicsScope instance that shares context with this scope
	...
	/**
	 * Pushes a symbol onto the execution context stack, creating a new execution context. Symbols represent GAML
	 * elements like statements, variables, or outputs.
	 *
	 * @param symbol
	 *            The symbol to push onto the context stack
	 */
	void push(ISymbol symbol);

	/**
	 * Removes a symbol from the execution context stack, reverting to the previous context. Should be called after
	 * execution of the symbol's code is complete.
	 *
	 * @param symbol
	 *            The symbol to remove from the context stack
	 */
	void pop(ISymbol symbol);

	/**
	 * Sets the current executing symbol without pushing it onto the stack. This is used for temporary symbol changes
	 * that don't require a new context.
	 *
	 * @param symbol
	 *            The symbol to set as the current executing symbol
	 */
	...
	/**
	 * Pushes a map of attributes onto the attribute stack. This is typically used when reading data from external
	 * sources like files or databases.
	 *
	 * @param values
	...
	/**
	 * Sets the value of a temporary variable used within iterators, such as the 'each' variable. These variables
	 * provide access to the current element in an iteration.
	 *
	 * @param name
	...
	/**
	 * Returns the root or top-level agent in the agent hierarchy for this scope. This is typically the experiment agent
	...
	/**
	 * Returns the simulation agent associated with this scope. The simulation agent manages the world and all the
	 * agents within it.
	 *
	 ...
	 * access to experiment parameters.
	 *
	...
	/**
	 * Removes an agent from the agent stack, reverting to the previous agent context. Should be called after execution
	 * in the agent's context is complete.
	 *
	 * @param iAgent
	...
	/**
	 * Returns a copy of the entire agent stack, representing the chain of agent contexts. The first element is the root
	 * agent, and the last element is the current agent.
	 *
	...
	/**
	 * Returns the random number generator associated with this scope. This generator is used for all stochastic
	...
	/**
	 * Returns the simulation clock associated with this scope. The clock manages time progression and scheduling in the
	 * simulation.
	 *
	 ...
	 *            The statement to execute
	 * @param agent
	 *            The agent in whose context to execute the statement
	 * @param args
	 *            The arguments to pass to the statement
	 * @return The result of the execution
	 */
	 ...
	 * @param agent
	 *            The agent in whose context to evaluate the expression
	...
	/**
	 * Evaluates an expression in the context of the specified object.
	 *
	 * @param expr
	 *            The expression to evaluate
	 * @param object
	 *            The object in whose context to evaluate the expression
	 * @return The result of the evaluation
	 * @throws GamaRuntimeException
	 *             If an error occurs during evaluation
	 */
	ExecutionResult evaluate(IExpression expr, IObject object) throws GamaRuntimeException;
	 ...
	 *
	 * @param agent
	 *            The agent whose variable to access
	 * @param name
	 *            The name of the variable
	 ...
	 *
	 * @param agent
	 *            The agent whose variable to modify
	 * @param name
	 *            The name of the variable
	 * @param v
	...
	/**
	 * Sets the value of a variable in the current execution context. This may be a local variable, an agent attribute,
	 * or a global variable. By default, variables defined in outer scopes can be modified.
	 *
	 * @param varName
	 *            The name of the variable
	 * @param val
	 ...
	 *            If true, only variables defined in this scope will be modified; if false, variables in outer scopes
	 *            can also be modified
	 */
	void setVarValue(String varName, Object val, boolean localScopeOnly);

	/**
	 * Saves all variable values from this scope into the provided map. This is typically used to capture the state of
	 * the scope for later restoration.
	 *
	 * @param varsToSave
	 *            The map in which to save the variable values
	 */
	void saveAllVarValuesIn(Map<String, Object> varsToSave);

	/**
	 * Removes all variables defined in this scope. This effectively clears the local variable context.
	 */
	void removeAllVars();
	...
	/**
	 * Returns the value of an argument, casting it to the specified type. Arguments are typically passed to actions or
	 * statements during execution.
	 *
	 * @param string
	 *            The name of the argument
	 * @param type
	 *            The GAML type ID to cast the value to
	 ...
	 *
	 * @param <T>
	 *            The Java type to cast the value to
	 * @param string
	 *            The name of the argument
	 * @param type
	 *            The GAML type ID to cast the value to
	...
	/**
	 * Returns the value of an argument cast to the specified Java type if it exists, or null if it doesn't.
	 *
	 * @param <T>
	 *            The Java type to cast the value to
	 * @param string
	...
	/**
	 * Returns the value of an argument cast to the specified Java type if it exists, or a default value if it doesn't.
	 *
	 * @param <T>
	 *            The Java type to cast the value to
	 * @param string
	 ...
	 *            The GAML type ID to cast the value to
	 * @param defaultValue
	 *            The value to return if the argument doesn't exist
	 ...
	 *
	 * @param <T>
	 *            The Java type of the list elements
	 * @param string
	 *            The name of the argument
	...
	/**
	 * Returns the value of an argument as a boolean if it exists, or a default value if it doesn't.
	 *
	 * @param string
	 *            The name of the argument
	 * @param ifNotExist
	...
	/**
	 * Returns the model associated with this scope. The model represents the entire GAML model being executed.
	 *
	 * @return The model
	 */
	IModel getModel();
	 ...
	 *
	 * @param status
	 *            The flow status to set
	 */
	void setFlowStatus(FlowStatus status);

	/**
	 * Initializes an agent that implements the IStepable interface. This is typically called when the agent is created.
	 *
	 * @param agent
	...
	/**
	 * Enables try mode for this scope, changing how errors are handled. In try mode, errors may be caught and handled
	...
	/**
	 * Executes a statement on an agent with control over scope usage and arguments. This is the most general form of
	 * execution, used by other execute methods.
	 *
	 * @param statement
	 *            The statement to execute
	 * @param target
	 *            The agent in whose context to execute the statement
	 * @param useTargetScopeForExecution
	 *            If true, use the target agent's scope; if false, use this scope
	 * @param args
	 *            The arguments to pass to the statement
	 * @return The result of the execution
	 */
	 ...
	 *
	 * @param comparison
	 *            The flow status to check for and clear
	...
	/**
	 * Sets the flow status to CONTINUE, which signals to skip to the next iteration of a loop.
	 */
	...
	/**
	 * Returns a piece of data associated with this scope by key. The default implementation returns null.
	 *
	 * @param key
	...
	/**
	 * Associates a piece of data with this scope by key. The default implementation does nothing.
	 *
	 * @param key
	 *            The key for the data
	 * @param value
	...
	/**
	 * Returns the population factory associated with this scope. The population factory is used to create new agents
	 * and populations.
	 *
	...
	/**
	 * Returns the server configuration associated with this scope. The default implementation delegates to the
	 * experiment scope.
	 *
	...
	/**
	 * Sets the server configuration associated with this scope. The default implementation delegates to the experiment
	 * scope.
	 *
	 * @param config
	...
	/**
	 * Returns the types manager associated with this scope. The types manager provides access to GAML types defined in
	 * the model.
	 *
