...
public class BufferingController {
	...
	public synchronized void flushAllBuffers() {
		...
		var agents = fileBufferPerAgentForCycles.entrySet().stream().map(s -> s.getValue().keySet())
				.flatMap(Collection::stream).toArray(length -> new AbstractAgent[length]);
		for (AbstractAgent agent : agents) { flushSaveFilesInCycle(agent); }
		...
		for (AbstractAgent agent : agents) { flushSaveFilesOfAgent(agent); }
	}
