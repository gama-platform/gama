		...
		switch (threshold) {
			case 0:
				for (final A a : array) {
					...
					theScope.execute(executable, agent, null);
					if (theScope.getAndClearBreakStatus() == FlowStatus.BREAK) { break; }
				}
				return;
			...
			case 1:
				for (final A a : array) {
					...
					IScope theScope = agent.getScope();
					if (theScope == null) { theScope = scope; }
					final IScope s = theScope;
					executeThreaded(() -> s.execute(executable, agent, null));
				}
				return;
			default:
