...
public class SimulationAgent extends GamlAgent implements ISimulationAgent {
	...
	final IProjectionFactory projectionFactory;
	...
	public void setTopology(final IScope scope, final IShape shape) {
		...
		if (!parallel[0]) {
			getSpecies().getDescription().visitMicroSpecies(s -> {
				parallel[0] = GamaExecutorService.getParallelism(scope, s.getFacetExpr(IKeyword.PARALLEL),
						Caller.SPECIES) > 0;
				return !parallel[0];
			});
		}
	...
	protected void postStep(final IScope scope) {
		...
		ownClock.step();
		BufferingUtils.getInstance().flushSaveFilesInCycle(this);
		BufferingUtils.getInstance().flushWriteInCycle(this);
	}
	 ...
	 */
	@Override
	public IProjectionFactory getProjectionFactory() { return projectionFactory; }
	...
	public void setGeometry(final IScope scope, final IShape g) {
		...
		if (projectionFactory.getWorld() == null) { projectionFactory.setWorldProjectionEnv(scope, env); }

		((WorldProjection) projectionFactory.getWorld()).updateTranslations(env);
		((WorldProjection) projectionFactory.getWorld()).updateUnit(projectionFactory.getUnitConverter());
		...
		if (projectionFactory.getUnitConverter() != null) {
			((WorldProjection) projectionFactory.getWorld()).convertUnit(geometry.getInnerGeometry());
	 ...
	 */
	@Override
	public IRandom generateRandomGenerator(final Double seed, final String rng) {
		setRandomGenerator(new RandomUtils(seed, rng));
		return random;
	}
	...
	public void updateWith(final IScope scope, final ISerialisedAgent sa) {
		...
		// Update RNG
		generateRandomGenerator(seedValue, rngValue);
		setUsage(usageValue);
