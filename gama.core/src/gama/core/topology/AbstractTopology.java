...
public abstract class AbstractTopology implements ITopology {
	...
	public Geometry returnToroidalGeom(final IPoint loc) {
		final List<Geometry> geoms = new ArrayList<>();
		final Point pt = GeometryUtils.getGeometryFactory().createPoint(loc.toCoordinate());
		final AffineTransformation at = new AffineTransformation();
		geoms.add(pt);
		for (int cnt = 0; cnt < 8; cnt++) {
			at.setToTranslation(getAdjustedXYVector()[cnt][0], getAdjustedXYVector()[cnt][1]);
		...
		return GeometryUtils.getGeometryFactory().buildGeometry(geoms);
	}
	...
	public Geometry returnToroidalGeom(final IShape shape) {
		...
		return GeometryUtils.getGeometryFactory().buildGeometry(listToroidalGeometries(shape.getInnerGeometry()));
	}
	...
	public Map<Geometry, IAgent> toroidalGeoms(final IScope scope, final IContainer<?, ? extends IShape> shps) {
		...
		for (final IShape ag : shps.iterable(scope)) {
			...
			if (agent != null) {
				geoms.put(GeometryUtils.getGeometryFactory()
	...
	public IPath pathBetween(final IScope scope, final IShape source, final IShape target) throws GamaRuntimeException {
		return GamaPathFactory.createFrom(scope, this,
	...
	public IPath pathBetween(final IScope scope, final IPoint source, final IPoint target) throws GamaRuntimeException {
		return GamaPathFactory.createFrom(scope, this, GamaListFactory.wrap(Types.POINT, source, target), 0.0);
	}
	...
	public void updateAgent(final IEnvelope previous, final IAgent agent) {
		...
		// }
		//
		// public void updateAgentBase(final GamaEnvelope previous, final IAgent agent) {
	...
	public IPoint getRandomLocation(final IScope scope) {
		return SpatialPunctal.any_location_in(scope, environment);
	}
