...
public class GraphTopology extends AbstractTopology {
	 ...
	 * @param torus
	 */
	public GraphTopology(final IScope scope, final IShape env, final ISpatialGraph graph) {
	...
	public IPath pathBetween(final IScope scope, final IShape source, final IShape target) {
		// final IPoint source = sourceShape.getLocation();
		...
		if (sourceNode && targetNode)
			return graph.getPathComputer().computeShortestPathBetween(scope, sourceN, targetN);
	 ...
	 */
	public IPath pathBetweenCommon(final IScope scope, final GamaSpatialGraph graph, final IShape edgeS,
		...
		return GamaPathFactory.createFrom(scope, this, source, target, edges);
	}
	 ...
	 * @return the gama spatial path
	 */
	IPath pathFromEdgesUndirected(final IScope scope, final IList<IShape> listOfEdges, final IShape edgeS,
			final IShape edgeT, final IShape source, final IShape target, final boolean sourceNode,
	 ...
	 * @return the gama spatial path
	 */
	public IPath pathBetweenCommonDirected(final IScope scope, final List<IShape> edgeS, final List<IShape> edgeT,
		...
		for (final IShape eS : edgeS) {
			for (final IShape eT : edgeT) {
				final IPath path = pathBetweenCommonDirected(scope, eS, eT, source, target, sourceNode, targetNode);
	 ...
	 * @return the gama spatial path
	 */
	public IPath pathBetweenCommonDirected(final IScope scope, final IShape edgeS, final IShape edgeT,
			final IShape source, final IShape target, final boolean sourceNode, final boolean targetNode) {
		IList<IShape> edges;

		if (!sourceNode && !targetNode && edgeS.equals(edgeT)) {
			final IPoint ptS = GamaPointFactory.create(edgeS.getInnerGeometry().getCoordinates()[0]);
	 ...
	 * @return the gama spatial path
	 */
	public IPath pathBetween(final IScope scope, final IShape source, final IShape target, final IShape existingEdge) {
		IList<IShape> edges;
	...
	@Override
	public IPath pathBetween(final IScope scope, final IPoint source, final IPoint target) {
	...
	public Double distanceBetween(final IScope scope, final IShape source, final IShape target) {
		final IPath path = this.pathBetween(scope, source, target);
		 ...
		 * Double.MAX_VALUE; } final IPoint sourceEdges = GamaPointFactory.create(coordsSource[0]); final IPoint
		 * targetEdges = GamaPointFactory.create(coordsTarget[coordsTarget.length - 1]); path.setSource(sourceEdges);
	...
	public Double directionInDegreesTo(final IScope scope, final IShape source, final IShape target) {
		...
		// final double result = MathOperators.atan2Opt(dy, dx);
		// return MathOperators.checkHeading((int) result);
	}
	...
	public IList KpathsBetweenCommon(final IScope scope, final IShape edgeS, final IShape edgeT, final IShape source,
		...
		for (final IList<IShape> edges : edgesList) {
			final IPath pp = pathFromEdgesUndirected(scope, edges, edgeS, edgeT, source, target, sourceNode, targetNode,
					nodeS, nodeSbis, nodeT, false);
	...
	public IList KpathsBetweenCommonDirected(final IScope scope, final IShape edgeS, final IShape edgeT,
		...
		if (edgeS.equals(edgeT)) {
			...
			if (source.euclidianDistanceTo(ptS) < target.euclidianDistanceTo(ptS)) {
				...
				edges.add(edgeS);
				results.add(GamaPathFactory.createFrom(scope, this, source, target, edges));
				return results;
			}
		}
		final IShape nodeS = sourceNode ? source : getPlaces().getEdgeTarget(edgeS);
		...
		for (IList<IShape> edges : edgesList) {
			...
			// return new GamaPath(this, source, target, edges);
			final IPath pp = GamaPathFactory.createFrom(scope, this, source, target, edges);
			results.add(pp);
		}
		Collections.sort(results);
		return results;
