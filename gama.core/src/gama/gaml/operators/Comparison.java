 ...
 *
 * <p>
 * This class implements the standard binary comparison operators for numeric, string, point, and generic object
 * operands:
 * </p>
 * <ul>
 ...
 *
 * <p>
 * <b>Nil handling:</b> for all numeric and point comparisons, a {@code nil} operand causes the operator to return
 * {@code false} immediately.
 * </p>
 *
 * <p>
 * <b>Floating-point equality:</b> the {@code =} operator for {@code float} values uses JTS's
 * {@link org.locationtech.jts.index.quadtree.IntervalSize#isZeroWidth} to determine equality within a tolerance based
 * on the magnitude of the operands. Two {@code NaN} values are considered equal by this implementation.
 * </p>
 *
 * <p>
 * <b>String comparisons:</b> all string comparison operators use Java's natural lexicographic order (case-sensitive,
 * Unicode code-point order).
 * </p>
 *
 * <p>
 * <b>Point comparisons:</b> point comparison uses a <em>partial order</em> where {@code p1 &lt; p2} iff
 * {@code p1.x &lt; p2.x} AND {@code p1.y &lt; p2.y}.
 * </p>
 *
...
public class Comparison {
	...
	/**
	 * Validates the operand types of an equality expression and emits a warning when the two types can never be equal
	 * (e.g. comparing an {@code int} with a {@code geometry}).
	 *
	 * <p>
	 * The validator does not prevent compilation; it only issues a {@link IGamlIssue#UNMATCHED_OPERANDS} warning so
	 * that the user is informed that the expression will always evaluate to {@code false}.
	 * </p>
	 */
	...
	@doc (
			...
			special_cases = { "If {@code inf > sup} (inverted bounds), the operator always returns {@code false}.",
					"If {@code a} equals {@code inf} or {@code sup} exactly, returns {@code false} (open interval)." },
			examples = { @example (
					value = "between(5, 1, 10)",
					equals = "true"),
					@example (
							value = "between(1, 1, 10)",
							equals = "false",
							isTestOnly = false),
					@example (
							value = "between(10, 1, 10)",
							equals = "false",
							isTestOnly = false),
					@example (
							value = "between(0, 5, 1)",
	...
	@doc (
			...
			examples = { @example (
					...
					equals = "true"),
					@example (
							value = "between(1.0, 1.0, 10.0)",
							equals = "false",
							isTestOnly = false),
					@example (
							value = "between(10.0, 1.0, 10.0)",
							equals = "false",
							isTestOnly = false) })
	...
	@operator (
			...
			concept = {})
	@doc (
			value = "Returns a binary matrix where each element is 1.0 if the corresponding matrix element is strictly greater than the scalar integer, and 0.0 otherwise.")
	...
	@operator (
			...
			concept = {})
	@doc (
			value = "Returns a binary matrix where each element is 1.0 if the corresponding matrix element is strictly less than the scalar integer, and 0.0 otherwise.")
	...
	@operator (
			...
			concept = {})
	@doc (
			value = "Returns a binary matrix where each element is 1.0 if the corresponding matrix element is equal to the scalar integer, and 0.0 otherwise.")
	...
	@doc (
			...
			returns = "a {@code bool}.",
			comment = """
					Float equality is determined using JTS's {@code IntervalSize.isZeroWidth}, \
			...
			special_cases = { "Two {@code NaN} values are considered equal by this implementation.",
	...
	@doc (
			...
			masterDoc = true,
			returns = "a {@code bool}.",
			special_cases = { "If both operands are {@code nil}, returns {@code true}.",
					"If one operand is {@code nil} and the other is not, returns {@code false}." },
			examples = { @example (
					value = "4 = 5",
					equals = "false"),
	...
	@doc (
			...
			masterDoc = true,
			returns = "a {@code bool}.",
			special_cases = { "If both operands are {@code nil}, returns {@code false} (they are equal).",
	...
	@doc (
			...
			special_cases = { "If the left operand is {@code nil}, returns {@code false}.",
	...
	@doc (
			...
			returns = "a {@code bool}.",
			comment = """
					Point comparison uses a <b>partial order</b>, not a total order. \
	...
	@doc (
			...
			returns = "a {@code bool}.",
			comment = "Point comparison uses a <b>partial order</b>, not a total order.",
			special_cases = { "If both points are identical, returns {@code true}.",
					"The z component is ignored in this comparison." },
			usages = { @usage (
