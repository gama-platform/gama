 ...
 *
 * <p>
 * Provides operators for creating, loading, saving, querying, filtering,
 * transforming, and combining tabular data (dataframes). Dataframes are
 * two-dimensional data structures with named columns and indexed rows, similar
 * to tables in databases or spreadsheets.
 * </p>
 *
 * @author GAMA Team
 */
public class DataFrameOperators {
	 ...
	 * </pre>
	 */
	@operator(value = "dataframe_with", can_be_const = false, type = IType.DATAFRAME, category = {
			IOperatorCategory.DATAFRAME }, concept = { IConcept.DATAFRAME, IConcept.CONTAINER })
	@doc(value = "Creates a dataframe from a list of column names and a list of rows (each row is a list of values).", usages = {
			@usage(value = "Create a dataframe with column names and data rows", examples = {
					@example(value = "dataframe_with([\"name\",\"age\"], [[\"Alice\",30],[\"Bob\",25]])", isExecutable = false) }) })
	 ...
	 */
	@operator(value = "load_table", can_be_const = false, type = IType.DATAFRAME, category = {
			IOperatorCategory.DATAFRAME, IOperatorCategory.FILE }, concept = { IConcept.DATAFRAME, IConcept.DATABASE })
	@doc(value = """
			...
			require credentials. The corresponding JDBC driver must be available on the classpath.""", usages = {
			@usage(value = "Load a PostgreSQL table", examples = {
					@example(value = "dataframe df <- load_table(\"jdbc:postgresql://localhost:5432/mydb\", \"user\", \"pwd\", \"people\");", isExecutable = false) }) }, see = {
							"load_sql", "save_table" })
	@no_test
	 ...
	 */
	@operator(value = "load_sql", can_be_const = false, type = IType.DATAFRAME, category = {
			IOperatorCategory.DATAFRAME, IOperatorCategory.FILE }, concept = { IConcept.DATAFRAME, IConcept.DATABASE })
	@doc(value = """
			...
			The corresponding JDBC driver must be available on the classpath.""", usages = {
			@usage(value = "Run a SQL query on a PostgreSQL database", examples = {
					@example(value = "dataframe df <- load_sql(\"jdbc:postgresql://localhost:5432/mydb\", \"user\", \"pwd\", \"SELECT name, age FROM people WHERE age > 18\");", isExecutable = false) }) }, see = {
	 ...
	 */
	@operator(value = "save_table", can_be_const = false, category = { IOperatorCategory.DATAFRAME,
			IOperatorCategory.FILE }, concept = { IConcept.DATAFRAME, IConcept.DATABASE })
	@doc(value = """
			...
			Pass empty strings for user/password if the database does not require credentials. \
			The corresponding JDBC driver must be available on the classpath. Returns true on success.""", usages = {
			@usage(value = "Save a dataframe to a PostgreSQL table", examples = {
					@example(value = "bool ok <- save_table(my_df, \"jdbc:postgresql://localhost:5432/mydb\", \"user\", \"pwd\", \"people\");", isExecutable = false) }) }, see = {
							"load_table", "load_sql" })
	@no_test
	...
	/**
	 * Converts an empty string to null, used to make user/password arguments
	 * optional in GAML.
	 */
	 ...
	 */
	@operator(value = "cell", can_be_const = true, category = { IOperatorCategory.DATAFRAME }, concept = {
			IConcept.DATAFRAME })
	@doc(value = "Returns the value at the specified row index and column name.", usages = {
			@usage(value = "Get a specific cell value", examples = {
	...
	// overloads registered for the DATAFRAME operand type and do not affect the
	// matrix versions.

	/**
	 * Returns the row at the given index as a list, mirroring the matrix
	 * {@code row_at} operator.
	 */
	@operator(value = "row_at", can_be_const = true, content_type = IType.NONE, type = IType.LIST, category = {
			IOperatorCategory.DATAFRAME }, concept = { IConcept.DATAFRAME })
	@doc(value = "Returns the row of the dataframe at the given index as a list of values. "
			+ "Overloads the matrix 'row_at' operator for dataframes.", usages = {
					@usage(value = "Get the row at index 1", examples = {
	...
	/**
	 * Returns a column by name as a list, mirroring the matrix {@code column_at}
	 * operator.
	 */
	@operator(value = "column_at", can_be_const = true, content_type = IType.NONE, type = IType.LIST, category = {
			IOperatorCategory.DATAFRAME }, concept = { IConcept.DATAFRAME })
	@doc(value = "Returns the values of the named column of the dataframe as a list. "
			+ "Overloads the matrix 'column_at' operator to accept a column name.", usages = {
					@usage(value = "Get the 'name' column", examples = {
							@example(value = "my_df column_at \"name\"", isExecutable = false) }) }, see = { "row_at",
									"columns_list" })
	...
	/**
	 * Returns a column by integer index as a list, mirroring the matrix
	 * {@code column_at} operator.
	 */
	@operator(value = "column_at", can_be_const = true, content_type = IType.NONE, type = IType.LIST, category = {
			IOperatorCategory.DATAFRAME }, concept = { IConcept.DATAFRAME })
	@doc(value = "Returns the values of the column at the given integer position as a list. "
			+ "Overloads the matrix 'column_at' operator for dataframes.", usages = {
					@usage(value = "Get the first column", examples = {
	...
	/**
	 * Returns all rows as a list of lists, mirroring the matrix {@code rows_list}
	 * operator.
	 */
	@operator(value = "rows_list", can_be_const = true, content_type = IType.LIST, type = IType.LIST, category = {
			IOperatorCategory.DATAFRAME }, concept = { IConcept.DATAFRAME })
	@doc(value = "Returns the list of the rows of the dataframe, each row being a list of its cell values. "
			+ "Overloads the matrix 'rows_list' operator for dataframes.", see = { "columns_list", "row_at" })
	...
	public static IList rowsList(final IScope scope, final IDataFrame df) {
		...
		for (int i = 0; i < df.getRows(); i++) {
			result.add(df.getRowValues(i));
		}
		return result;
	}

	/**
	 * Returns all columns as a list of lists, mirroring the matrix
	 * {@code columns_list} operator.
	 */
	@operator(value = "columns_list", can_be_const = true, content_type = IType.LIST, type = IType.LIST, category = {
			IOperatorCategory.DATAFRAME }, concept = { IConcept.DATAFRAME })
	@doc(value = "Returns the list of the columns of the dataframe, each column being a list of its values. "
			+ "Overloads the matrix 'columns_list' operator for dataframes.", see = { "rows_list", "column_at" })
	...
	public static IList columnsList(final IScope scope, final IDataFrame df) {
		final IList result = GamaListFactory.create(Types.LIST);
		for (final String col : df.getColumns()) {
			result.add(df.getColumnValues(col));
		}
		return result;
	}
	 ...
	 */
	@operator(value = "filter", can_be_const = true, type = IType.DATAFRAME, category = {
			IOperatorCategory.DATAFRAME }, concept = { IConcept.DATAFRAME, IConcept.FILTER })
	@doc(value = "Returns a new dataframe containing only the rows where the specified column equals the given value.", usages = {
			@usage(value = "Filter rows where 'city' equals 'Paris'", examples = {
					@example(value = "dataframe df2 <- filter(my_df, \"city\", \"Paris\");", isExecutable = false) }) }, see = {
							"remove_empty", "select_columns" })
	 ...
	 */
	@operator(value = "remove_empty", can_be_const = true, type = IType.DATAFRAME, category = {
			IOperatorCategory.DATAFRAME }, concept = { IConcept.DATAFRAME, IConcept.FILTER })
	@doc(value = "Returns a new dataframe with rows removed where the specified column has null or empty values.", usages = {
			@usage(value = "Remove rows with empty 'name' values", examples = {
					@example(value = "dataframe df2 <- remove_empty(my_df, \"name\");", isExecutable = false) }) }, see = {
	 ...
	 */
	@operator(value = "select_columns", can_be_const = true, type = IType.DATAFRAME, category = {
			IOperatorCategory.DATAFRAME }, concept = { IConcept.DATAFRAME })
	@doc(value = "Returns a new dataframe containing only the specified columns.", usages = {
			@usage(value = "Select only 'name' and 'age' columns", examples = {
					@example(value = "dataframe df2 <- select_columns(my_df, [\"name\", \"age\"]);", isExecutable = false) }) }, see = {
							"filter", "add_column" })
	 ...
	 */
	@operator(value = "add_column", can_be_const = true, type = IType.DATAFRAME, category = {
			IOperatorCategory.DATAFRAME }, concept = { IConcept.DATAFRAME })
	@doc(value = "Returns a new dataframe with an additional column filled with the given default value.", usages = {
			@usage(value = "Add a 'score' column with default value 0", examples = {
					@example(value = "dataframe df2 <- add_column(my_df, \"score\", 0);", isExecutable = false) }) }, see = {
							"select_columns" })
	 ...
	 */
	@operator(value = "join", can_be_const = true, type = IType.DATAFRAME, category = {
			IOperatorCategory.DATAFRAME }, concept = { IConcept.DATAFRAME })
	@doc(value = "Inner-joins two dataframes on the given common key column: only the rows whose key matches in "
			+ "both dataframes are kept.", usages = {
					@usage(value = "Join two dataframes on the 'id' column", examples = {
							@example(value = "join(df_people, df_scores, \"id\")", isExecutable = false) }) }, see = {
									"pivot" })
	...
	@operator(value = "join", can_be_const = true, type = IType.DATAFRAME, category = {
			IOperatorCategory.DATAFRAME }, concept = { IConcept.DATAFRAME })
	@doc(value = "Inner-joins two dataframes on the given list of common key columns.", usages = {
			@usage(value = "Join two dataframes on two key columns", examples = {
					@example(value = "join(df1, df2, [\"country\", \"year\"])", isExecutable = false) }) }, see = {
							"join" })
	...
	@operator(value = "join", can_be_const = true, type = IType.DATAFRAME, category = {
			IOperatorCategory.DATAFRAME }, concept = { IConcept.DATAFRAME })
	@doc(value = "Joins two dataframes on the given key columns using an explicit join type: \"inner\" (default), "
			+ "\"left\", \"right\" or \"full\".", usages = {
					@usage(value = "Left-join two dataframes on 'id' (keeps every row of the left dataframe)", examples = {
							@example(value = "join(df1, df2, [\"id\"], \"left\")", isExecutable = false) }) }, see = {
									"join" })
	 ...
	 */
	@operator(value = IKeyword.PLUS, can_be_const = true, type = IType.DATAFRAME, category = {
			IOperatorCategory.DATAFRAME }, concept = { IConcept.DATAFRAME })
	@doc(value = "Vertically concatenates two dataframes (appends the rows of the second to the first). "
			+ "Both dataframes must share the same column structure.", see = { "join" })
	...
	/**
	 * Appends a single row (given as a list of values) to a dataframe with the '+'
	 * operator.
	 */
	@operator(value = IKeyword.PLUS, can_be_const = true, type = IType.DATAFRAME, category = {
			IOperatorCategory.DATAFRAME }, concept = { IConcept.DATAFRAME })
	@doc(value = "Appends a single row to the dataframe. The list of values must match the number of columns.", see = {
			"join" })
	 ...
	 * Pivots a dataframe.
	 */
	@operator(value = "pivot", can_be_const = true, type = IType.DATAFRAME, category = {
			IOperatorCategory.DATAFRAME }, concept = { IConcept.DATAFRAME })
	@doc(value = "Pivots the dataframe: the index column becomes row labels, the pivot column values become new column names, "
			+ "and the value column provides the cell values. When multiple values exist for a combination, the first is kept.", usages = {
					@usage(value = "Pivot a sales dataframe", examples = {
							@example(value = "dataframe pivoted <- pivot(sales_df, \"product\", \"quarter\", \"revenue\");", isExecutable = false) }) }, see = {
									"filter", "select_columns" })
	...
	@operator(value = "pretty_print", can_be_const = true, type = IType.STRING, category = {
			IOperatorCategory.DATAFRAME }, concept = { IConcept.DATAFRAME })
	@doc(value = "Creates a string representing the dataframe in a human readable format. The number of rows and columns is limited to 10 and the number of characters per cell to 50.")
	 ...
	 */
	@operator(value = "iloc", can_be_const = true, content_type = IType.NONE, type = IType.LIST, category = {
			IOperatorCategory.DATAFRAME }, concept = { IConcept.DATAFRAME })
	@doc(value = "Pandas-style df.iloc[i]: returns the row at integer position i as a list of values. "
			+ "Negative indices are supported (-1 is the last row).", usages = {
					@usage(value = "Get the last row", examples = {
							@example(value = "list row <- iloc(my_df, -1);", isExecutable = false) }) }, see = {
									"row_at", "cell" })
	...
	/**
	 * Pandas-style {@code df.iloc[i, j]}: returns a single cell value at the given
	 * integer (row, col) position.
	 */
	@operator(value = "iloc", can_be_const = true, category = { IOperatorCategory.DATAFRAME }, concept = {
			IConcept.DATAFRAME })
	@doc(value = "Pandas-style df.iloc[i, j]: returns the cell value at integer (row, col) position. "
			+ "Negative indices are supported on both axes.", usages = {
					@usage(value = "Get the cell at row 1, column 0", examples = {
							@example(value = "unknown v <- iloc(my_df, 1, 0);", isExecutable = false) }) }, see = {
									"cell", "row_at", "column_at" })
	...
	/**
	 * Pandas-style {@code df.iloc[i, [j, ...]]}: returns row i restricted to the
	 * selected columns, as a list.
	 */
	@operator(value = "iloc", can_be_const = true, content_type = IType.NONE, type = IType.LIST, category = {
			IOperatorCategory.DATAFRAME }, concept = { IConcept.DATAFRAME })
	@doc(value = "Pandas-style df.iloc[i, [j, ...]]: returns the values of row i taken from the selected columns, "
			+ "in the order of the input column indices. Negative indices are supported.", usages = {
					@usage(value = "Get cells [0] and [2] of row 1", examples = {
	...
	/**
	 * Pandas-style {@code df.iloc[[i, ...], j]}: returns column j restricted to the
	 * selected rows, as a list.
	 */
	@operator(value = "iloc", can_be_const = true, content_type = IType.NONE, type = IType.LIST, category = {
			IOperatorCategory.DATAFRAME }, concept = { IConcept.DATAFRAME })
	@doc(value = "Pandas-style df.iloc[[i, ...], j]: returns the values of column j taken from the selected rows, "
			+ "in the order of the input row indices. Negative indices are supported.", usages = {
					@usage(value = "Get column 1 values at rows 0 and 2", examples = {
	...
	/**
	 * Pandas-style {@code df.iloc[[i, ...]]}: returns a sub-dataframe with the
	 * selected rows (all columns kept).
	 */
	@operator(value = "iloc", can_be_const = true, type = IType.DATAFRAME, category = {
			IOperatorCategory.DATAFRAME }, concept = { IConcept.DATAFRAME })
	@doc(value = "Pandas-style df.iloc[[i, ...]]: returns a new dataframe containing only the rows at the given "
			+ "integer indices (in that order). All columns are kept. Negative indices are supported.", usages = {
					@usage(value = "Select rows 0 and 2", examples = {
							@example(value = "dataframe sub <- iloc(my_df, [0, 2]);", isExecutable = false) }) }, see = {
									"row_at", "filter" })
	...
	/**
	 * Pandas-style {@code df.iloc[[i, ...], [j, ...]]}: returns a sub-dataframe
	 * with the selected rows and columns.
	 */
	@operator(value = "iloc", can_be_const = true, type = IType.DATAFRAME, category = {
			IOperatorCategory.DATAFRAME }, concept = { IConcept.DATAFRAME })
	@doc(value = """
			...
			supported on both axes.""", usages = { @usage(value = "Select rows 0 and 2, columns 0 and 1", examples = {
			@example(value = "dataframe sub <- iloc(my_df, [0, 2], [0, 1]);", isExecutable = false) }) }, see = {
