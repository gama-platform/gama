...
/**
 * Provides GAML field/grid operators for operating on {@link IField} (raster/field matrix) objects within the GAMA
 * modeling platform. An {@link IField} is a matrix-like raster data structure used for grid-based spatial computations,
 * where each cell stores a floating-point value and is associated with a rectangular geographic area.
 *
 * <p>
 * The following operators are provided:
 * </p>
 * <ul>
 * <li><strong>{@code cell_at}</strong> – returns the rectangular cell shape at a given world-coordinate location or
 * (column, row) matrix index.</li>
 * <li><strong>{@code cells_in}</strong> – returns the list of cell shapes whose centres are inside a given geometry
 * (centre-point intersection).</li>
 * <li><strong>{@code cells_overlapping}</strong> – returns the list of cell shapes that geometrically overlap a given
 * geometry (more precise but slower than {@code cells_in}).</li>
 * <li><strong>{@code values_in}</strong> – returns the list of field values for all cells whose centres intersect a
 * given geometry.</li>
 * <li><strong>{@code points_in}</strong> – returns the list of cell-centre points for all cells whose centres intersect
 * a given geometry.</li>
 * <li><strong>{@code neighbors_of}</strong> – returns the world-coordinate centres of the cells surrounding a given
 * location.</li>
 * </ul>
 *
 * <p>
 * All operators are annotated {@code @no_test} because they require an active simulation with a properly initialised
 * field that is not available during standalone unit testing.
 * </p>
 *
...
public class Fields {
	...
	@operator (
			...
			doc = { @doc (
					...
					special_cases = { "Returns nil if the location is outside the field boundaries." }) })
	@no_test
	...
	@operator (
			...
			doc = { @doc (
					...
					special_cases = { "Returns nil if the column or row index is outside the field dimensions." }) })
	@no_test
	...
	@operator (
			...
			doc = { @doc (
					...
					special_cases = { "Cells on the boundary of the field have fewer neighbours.",
	...
	@operator (
			...
			doc = { @doc (
					...
					examples = { @example (
							...
							isExecutable = false) }) })
	public static IMatrix shiftMatrix(final IScope scope, final IMatrix matrix, final Integer dx, final Integer dy,
			final Boolean wrap) {
	...
	public static IMatrix matrixWith(final IScope scope, final Object pop, final String varName) {
		...
		if (pop instanceof IPopulation.Grid g) {
			grid = g;
		} else if (pop instanceof ISpecies species && species.isGrid()) {
			grid = (IPopulation.Grid) scope.getSimulation().getPopulationFor(species);
		}
		if (grid == null)
			throw GamaRuntimeException.error("The target must be a grid species or a grid population", scope);
		...
		g.getValuesInto(scope, varName, matrix.getMatrix());
		return matrix;
	}
	...
	@operator (
			...
			doc = { @doc (
					...
					examples = { @example (
							...
							isExecutable = false) }) })
	public static IPopulation.Grid setValues(final IScope scope, final Object pop, final String varName,
		...
		for (int i = 0; i < size; i++) { g.setValueAtIndex(scope, i, varName, data[i]); }
		return grid;
	}
