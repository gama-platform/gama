/*******************************************************************************************************
 *
 * PaletteBasedMeshColorProvider.java, in gama.core, is part of the source code of the GAMA modeling and simulation
 * platform (v.2025-03).
 *
 * (c) 2007-2026 UMI 209 UMMISCO IRD/SU & Partners (IRIT, MIAT, ESPACE-DEV, CTU)
 *
 * Visit https://github.com/gama-platform/gama for license information and contacts.
 *
 ********************************************************************************************************/
package gama.gaml.statements.draw;

import gama.api.ui.layers.IMeshColorProvider;
import gama.core.util.color.GamaPalette;

/**
 * The Class PaletteBasedMeshColorProvider.
 */
public class PaletteBasedMeshColorProvider implements IMeshColorProvider {

	/** The components. */
	final double[] components;

	/** The size. */
	final int size;

	/**
	 * Instantiates a new palette based mesh color provider.
	 *
	 * @param palette
	 *            the palette
	 */
	public PaletteBasedMeshColorProvider(final GamaPalette palette) {
		this.size = palette.size();
		components = new double[size * 4];
		for (var i = 0; i < size; i++) {
			var c = palette.get(i);
			components[i * 4] = c.red() / 255d;
			components[i * 4 + 1] = c.green() / 255d;
			components[i * 4 + 2] = c.blue() / 255d;
			components[i * 4 + 3] = c.alpha() / 255d;
		}
	}

	@Override
	public double[] getColor(final int index, final double z, final double min, final double max, final double[] rgb) {
		var result = rgb;
		if (result == null) { result = newArray(); }
		if (min > max || z <= min) {
			result[0] = components[0];
			result[1] = components[1];
			result[2] = components[2];
			result[3] = components[3];
		} else if (z >= max) { // Can happen if multiple threads write and read the field !
			result[0] = components[4 * (size - 1)];
			result[1] = components[4 * (size - 1) + 1];
			result[2] = components[4 * (size - 1) + 2];
			result[3] = components[4 * (size - 1) + 3];
		} else {
			double intervalSize = (max - min) / (size - 1);
			double intervald = (z - min) / intervalSize;
			int intervali = (int) intervald;
			double r = intervald - intervali, ir = 1d - r;
			result[0] = components[intervali * 4] * ir + components[(intervali + 1) * 4] * r;
			result[1] = components[intervali * 4 + 1] * ir + components[(intervali + 1) * 4 + 1] * r;
			result[2] = components[intervali * 4 + 2] * ir + components[(intervali + 1) * 4 + 2] * r;
			result[3] = components[intervali * 4 + 3] * ir + components[(intervali + 1) * 4 + 3] * r;
		}
		return result;
	}

}
