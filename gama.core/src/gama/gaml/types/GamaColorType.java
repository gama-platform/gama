...
public class GamaColorType extends GamaType<IColor> {
	 ...
	 */
	public static IColor staticCast(final IScope scope, final Object obj, final Object param, final boolean copy)
			throws GamaRuntimeException {
		...
		switch (obj) {
			...
			case GamaColor col -> {
				if (param instanceof Integer a) return GamaColorFactory.get(col.red(), col.green(), col.blue(), a);
				if (param instanceof Double a)
					return GamaColorFactory.getWithDoubleAlpha(col.red(), col.green(), col.blue(), a);
		...
		if (obj instanceof String) {
			...
			IColor c = GamaColorFactory.COLORS.get(s);
			if (c == null) {
				try {
			...
			switch (param) {
				...
				case Integer i -> {
					return GamaColorFactory.createWithAlpha(c, i);
				}
				case Double d -> {
					return GamaColorFactory.createWithAlpha(c, d);
				}
				default -> {
				}
			}
