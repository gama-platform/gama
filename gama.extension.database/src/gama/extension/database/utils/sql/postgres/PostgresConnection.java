/*******************************************************************************************************
 *
 * PostgresConnection.java, in gama.extension.database, is part of the source code of the GAMA modeling and simulation
 * platform (v.2025-03).
 *
...
public class PostgresConnection extends SqlConnection {
	 ...
	 *            the transformed
	 */
	public PostgresConnection(final IScope scope, final String venderName, final String url, final String port,
	...
	protected String getInsertString(final IScope scope, final Connection conn, final String table_name,
		...
		for (int i = 0; i < col_no; i++) {
			...
			if (i < col_no - 1) { colStr.append(","); }
		}
		// create SELECT statement string
		selectStr = selectStr + colStr + " FROM " + table_name + " LIMIT 1 ;";

		if (DEBUG.IS_ON()) { DEBUG.OUT("PostgresConnection.getInsertString.select command:" + selectStr); }
		...
		try {
			...
			if (DEBUG.IS_ON()) { DEBUG.OUT("PostgresConection.getInsertString:" + insertStr); }
