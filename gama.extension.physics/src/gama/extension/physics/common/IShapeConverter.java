/*******************************************************************************************************
 *
 * IShapeConverter.java, in gaml.extensions.physics, is part of the source code of the GAMA modeling and simulation
 * platform .
 *
...
public interface IShapeConverter<ShapeType, VectorType> extends IPhysicalEntity<VectorType> {
	...
	default float computeDepth(final IAgent agent) {
		...
		if (agent.getSpecies().isGrid()) {
			result = (float) ((IGrid) agent.getPopulation().getTopology().getPlaces()).getValue(agent.getIndex());
		} else {
