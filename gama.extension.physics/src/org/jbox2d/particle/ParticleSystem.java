/*******************************************************************************************************
 *
 * ParticleSystem.java, in gaml.extensions.physics, is part of the source code of the GAMA modeling and
...
public class ParticleSystem {
	...
	public int createParticle(final ParticleDef def) {
		if (m_count >= m_internalAllocatedCapacity) {
			...
			if (m_internalAllocatedCapacity < capacity) {
				...
				m_depthBuffer =
						BufferUtils.reallocateBuffer(m_depthBuffer, 0, m_internalAllocatedCapacity, capacity, true);
		...
		if (m_count >= m_internalAllocatedCapacity) return Settings.invalidParticleIndex;
		int index = m_count++;
		...
		if (m_depthBuffer != null) { m_depthBuffer[index] = 0; }
	...
	public void destroyParticle(final int index, final boolean callDestructionListener) {
		...
		if (callDestructionListener) { flags |= ParticleType.b2_destructionListener; }
	...
	public void destroyParticlesInGroup(final ParticleGroup group, final boolean callDestructionListener) {
		for (int i = group.m_firstIndex; i < group.m_lastIndex; i++) { destroyParticle(i, callDestructionListener); }
	}
	...
	public ParticleGroup createParticleGroup(final ParticleGroupDef groupDef) {
		...
		if (m_groupList != null) { m_groupList.m_prev = group; }
		m_groupList = group;
		++m_groupCount;
		for (int i = firstIndex; i < lastIndex; i++) { m_groupBuffer[i] = group; }
		...
		if ((groupDef.flags & k_triadFlags) != 0) {
			...
			for (int i = firstIndex; i < lastIndex; i++) { diagram.addGenerator(m_positionBuffer.data[i], i); }
		...
		if ((groupDef.groupFlags & ParticleGroupType.b2_solidParticleGroup) != 0) { computeDepthForGroup(group); }
	...
	public void joinParticleGroups(final ParticleGroup groupA, final ParticleGroup groupB) {
		...
		int particleFlags = 0;
		for (int i = groupA.m_firstIndex; i < groupB.m_lastIndex; i++) { particleFlags |= m_flagsBuffer.data[i]; }
		...
		for (int i = groupB.m_firstIndex; i < groupB.m_lastIndex; i++) { m_groupBuffer[i] = groupA; }
		...
		if ((groupFlags & ParticleGroupType.b2_solidParticleGroup) != 0) { computeDepthForGroup(groupA); }
	}
	...
	void destroyParticleGroup(final ParticleGroup group) {
		...
		for (int i = group.m_firstIndex; i < group.m_lastIndex; i++) { m_groupBuffer[i] = null; }

		if (group.m_prev != null) { group.m_prev.m_next = group.m_next; }
		if (group.m_next != null) { group.m_next.m_prev = group.m_prev; }
		if (group == m_groupList) { m_groupList = group.m_next; }
	...
	public void computeDepthForGroup(final ParticleGroup group) {
		for (int i = group.m_firstIndex; i < group.m_lastIndex; i++) { m_accumulationBuffer[i] = 0; }
		for (int k = 0; k < m_contactCount; k++) {
			final ParticleContact contact = m_contactBuffer[k];
			int a = contact.indexA;
			int b = contact.indexB;
	...
	public void addContact(final int a, final int b) {
		...
		if (d2 < m_squaredDiameter) {
			if (m_contactCount >= m_contactCapacity) {
				...
				m_contactBuffer =
						BufferUtils.reallocateBuffer(ParticleContact.class, m_contactBuffer, oldCapacity, newCapacity);
	...
	public void solve(final TimeStep step) {
		...
		if (m_count == 0) return;
		m_allParticleFlags = 0;
		for (int i = 0; i < m_count; i++) { m_allParticleFlags |= m_flagsBuffer.data[i]; }
		if ((m_allParticleFlags & ParticleType.b2_zombieParticle) != 0) { solveZombie(); }
		if (m_count == 0) return;
		m_allGroupFlags = 0;
		...
		solveCollision(step);
		if ((m_allGroupFlags & ParticleGroupType.b2_rigidParticleGroup) != 0) { solveRigid(step); }
		if ((m_allParticleFlags & ParticleType.b2_wallParticle) != 0) { solveWall(step); }
		for (int i = 0; i < m_count; i++) {
			Vec2 pos = m_positionBuffer.data[i];
		...
		if ((m_allParticleFlags & ParticleType.b2_viscousParticle) != 0) { solveViscous(step); }
		if ((m_allParticleFlags & ParticleType.b2_powderParticle) != 0) { solvePowder(step); }
		if ((m_allParticleFlags & ParticleType.b2_tensileParticle) != 0) { solveTensile(step); }
		if ((m_allParticleFlags & ParticleType.b2_elasticParticle) != 0) { solveElastic(step); }
		if ((m_allParticleFlags & ParticleType.b2_springParticle) != 0) { solveSpring(step); }
		if ((m_allGroupFlags & ParticleGroupType.b2_solidParticleGroup) != 0) { solveSolid(step); }
		if ((m_allParticleFlags & ParticleType.b2_colorMixingParticle) != 0) { solveColorMixing(step); }
		solvePressure(step);
		solveDamping(step);
	}
	...
	void solvePressure(final TimeStep step) {
		...
		for (int i = 0; i < m_count; i++) { m_accumulationBuffer[i] = 0; }
		...
		if ((m_allParticleFlags & k_noPressureFlags) != 0) {
			for (int i = 0; i < m_count; i++) {
				if ((m_flagsBuffer.data[i] & k_noPressureFlags) != 0) { m_accumulationBuffer[i] = 0; }
			}
		}
	...
	void solveZombie() {
		...
		for (int i = 0; i < m_count; i++) {
			...
			if ((flags & ParticleType.b2_zombieParticle) != 0) {
				...
				if (i != newCount) {
					...
					if (m_depthBuffer != null) { m_depthBuffer[newCount] = m_depthBuffer[i]; }
					if (m_colorBuffer.data != null) { m_colorBuffer.data[newCount].set(m_colorBuffer.data[i]); }
					if (m_userDataBuffer.data != null) { m_userDataBuffer.data[newCount] = m_userDataBuffer.data[i]; }
				}
				newCount++;
			}
		}
		...
		for (ParticleGroup group = m_groupList; group != null; group = group.getNext()) {
			...
			if (firstIndex < lastIndex) {
				...
				group.m_lastIndex = 0;
				if (group.m_destroyAutomatically) { group.m_toBeDestroyed = true; }
			}
		}
	...
	void RotateBuffer(final int start, final int mid, final int end) {
		...
		if (m_depthBuffer != null) { BufferUtils.rotate(m_depthBuffer, start, mid, end); }
		if (m_colorBuffer.data != null) { BufferUtils.rotate(m_colorBuffer.data, start, mid, end); }
		if (m_userDataBuffer.data != null) { BufferUtils.rotate(m_userDataBuffer.data, start, mid, end); }
	 ...
	 */
	public float getParticleDensity() { return m_density; }

	/**
	 * Sets the particle gravity scale.
	 *
	 * @param gravityScale
	 *            the new particle gravity scale
	 */
	public void setParticleGravityScale(final float gravityScale) { m_gravityScale = gravityScale; }
	 ...
	 */
	public float getParticleGravityScale() { return m_gravityScale; }

	/**
	 * Sets the particle damping.
	 *
	 * @param damping
	 *            the new particle damping
	 */
	public void setParticleDamping(final float damping) { m_dampingStrength = damping; }
	 ...
	 */
	public float getParticleDamping() { return m_dampingStrength; }
	 ...
	 */
	public float getParticleRadius() { return m_particleDiameter / 2; }
	 ...
	 */
	float getParticleStride() { return Settings.particleStride * m_particleDiameter; }
	 ...
	 */
	float getParticleInvMass() { return 1.777777f * m_inverseDensity * m_inverseDiameter * m_inverseDiameter; }
	 ...
	 */
	public int[] getParticleFlagsBuffer() { return m_flagsBuffer.data; }
	 ...
	 */
	public Vec2[] getParticlePositionBuffer() { return m_positionBuffer.data; }
	 ...
	 */
	public Vec2[] getParticleVelocityBuffer() { return m_velocityBuffer.data; }
	 ...
	 */
	public int getParticleMaxCount() { return m_maxCount; }

	/**
	 * Sets the particle max count.
	 *
	 * @param count
	 *            the new particle max count
	 */
	 ...
	 */
	public ParticleGroup[] getParticleGroupBuffer() { return m_groupBuffer; }
	 ...
	 */
	public int getParticleGroupCount() { return m_groupCount; }
	 ...
	 */
	public ParticleGroup[] getParticleGroupList() { return m_groupBuffer; }
	 ...
	 */
	public int getParticleCount() { return m_count; }
	 ...
	 */
	// reallocate a buffer
	static <T> T[] reallocateBuffer(final ParticleBuffer<T> buffer, final int oldCapacity, final int newCapacity,
	 ...
	 * @return the int[]
	 */
	static int[] reallocateBuffer(final ParticleBufferInt buffer, final int oldCapacity, final int newCapacity,
			final boolean deferred) {
		assert newCapacity > oldCapacity;
	...
	float[] requestParticleBuffer(float[] buffer) {
		if (buffer == null) { buffer = new float[m_internalAllocatedCapacity]; }
		return buffer;
	}
		...
		public void callback(final int a, final int b, final int c) {
			...
			if (dabx * dabx + daby * daby < maxDistanceSquared && dbcx * dbcx + dbcy * dbcy < maxDistanceSquared
				...
				if (system.m_triadCount >= system.m_triadCapacity) {
					...
					int newCapacity =
							system.m_triadCount != 0 ? 2 * system.m_triadCount : Settings.minParticleBufferCapacity;
					system.m_triadBuffer =
							BufferUtils.reallocateBuffer(Triad.class, system.m_triadBuffer, oldCapacity, newCapacity);
				...
				triad.indexC = c;
				triad.flags =
						system.m_flagsBuffer.data[a] | system.m_flagsBuffer.data[b] | system.m_flagsBuffer.data[c];
	...
	static class DestroyParticlesInShapeCallback implements ParticleQueryCallback {
		 ...
		 *            the call destruction listener
		 */
		public void init(final ParticleSystem system, final Shape shape, final Transform xf,
