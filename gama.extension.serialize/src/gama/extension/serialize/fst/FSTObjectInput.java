...
public class FSTObjectInput implements ObjectInput {
	...
	protected Object instantiateEnum(final FSTClazzInfo.FSTFieldInfo referencee, final int readPos)
		...
		FSTClazzInfo clzSerInfo = readClass();
		// c = clzSerInfo.getClazz();
	...
	protected Object instantiateAndReadNoSer(final Class<?> c, final FSTClazzInfo clzSerInfo,
			final FSTClazzInfo.FSTFieldInfo referencee, final int readPos) throws Exception {
		Object newObj = clzSerInfo.newInstance(getCodec().isMapBased());
		if (newObj == null) throw new IOException(referencee.getDesc() + ":Failed to instantiate '" + c.getName()
				+ "'. Register a custom serializer implementing instantiate or define empty constructor.");
			...
			if (isUnknown) {
				...
				if (fieldInfo == null) {
					...
					switch (fieldInfo.getIntegralType()) {
						case FSTClazzInfo.FSTFieldInfo.BOOL -> fieldInfo.setBooleanValue(newObj,
								getCodec().readFByte() != 0);
						case FSTClazzInfo.FSTFieldInfo.BYTE -> fieldInfo.setByteValue(newObj, getCodec().readFByte());
						case FSTClazzInfo.FSTFieldInfo.CHAR -> fieldInfo.setCharValue(newObj, getCodec().readFChar());
						case FSTClazzInfo.FSTFieldInfo.SHORT -> fieldInfo.setShortValue(newObj,
								getCodec().readFShort());
						case FSTClazzInfo.FSTFieldInfo.INT -> fieldInfo.setIntValue(newObj, getCodec().readFInt());
						case FSTClazzInfo.FSTFieldInfo.LONG -> fieldInfo.setLongValue(newObj, getCodec().readFLong());
						case FSTClazzInfo.FSTFieldInfo.FLOAT -> fieldInfo.setFloatValue(newObj,
								getCodec().readFFloat());
						case FSTClazzInfo.FSTFieldInfo.DOUBLE -> fieldInfo.setDoubleValue(newObj,
								getCodec().readFDouble());
