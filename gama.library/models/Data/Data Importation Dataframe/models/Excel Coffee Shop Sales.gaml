...
global {
	...
	init {
		...
		write "Types   : " + df_column_types(transactions);
		write "";
		...
		map<string,int> type_counts <- map([]);
...
experiment "Explore Coffee Sales" type: gui {
	...
	output {
		...
		display "Revenue by Category" type: 2d {
			chart "Revenue by Product Category" type: bar {
		...
		display "Avg Price by Category" type: 2d {
			chart "Average Unit Price by Product Category" type: bar {
				loop i from: 0 to: df_rows(by_category) - 1 {
					data string(df_cell(by_category, i, "category"))
