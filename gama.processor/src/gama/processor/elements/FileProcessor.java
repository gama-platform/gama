 ...
 * 
 * <p>File processors handle the registration of custom file type implementations that can be used to
 * read, write, and manipulate files in GAMA simulations. File types define how specific file formats
 * are loaded, processed, and converted into usable data structures within the simulation environment.
 * 
 ...
 * <p>A file type definition includes:
 * <ul>
...
public class FileProcessor extends ElementProcessor<file> {
	...
	private static final String FILE_TYPE_VERIFICATION_TEMPLATE = 
			...
			+ "(s,o)-> { return GamaFileType.verifyExtension(%s,Cast.asString(s, o[0]));}, false);";

	@Override
	public void createElement(final StringBuilder sb, final Element e, final file f) {
		final String name = f.name();
		verifyDoc(e, "file declaration " + name, f);
	...
	private void createFileTypeVerificationOperator(final StringBuilder sb, final String name) {
		sb.append(in).append("_operator(S(").append(toJavaString("is_" + name))
				.append(String.format(FILE_TYPE_VERIFICATION_TEMPLATE, name, toJavaString(name)));
	}
	 ...
	 *
	 * @param sb the StringBuilder to append to
	 * @param e the element representing the file class
	 * @param name the file name
	 * @param clazz the class name
	 * @param f the file annotation
	 */
	private void processConstructors(final StringBuilder sb, final Element e, final String name, 
	 ...
	 * @param f the file annotation
	 */
	private void processConstructor(final StringBuilder sb, final Element constructor, 
			final String name, final String clazz, final file f) {
		...
		for (int i = 1; i < n; i++) {
			...
			if (context.isIType(type)) { 
				indexOfIType = i; 
			}
			args[i - 1] = rawNameOf(type);
		}
		
		final int content = indexOfIType == -1 ? f.buffer_content() 
				: ITypeProvider.DENOTED_TYPE_AT_INDEX + indexOfIType;
	 ...
	 * @param name the file name
	 * @param clazz the class name
	 * @param names the parameter types
	 * @param forcedContent the forced content value
	...
	private void writeCreateFileOperator(final StringBuilder sb, final String name, final String clazz,
		...
		for (final String classe : names) { 
			sb.append(',').append(toClassObject(classe)); 
		}
	...
	protected boolean validateElement(final Element e) {
		...
		result &= assertOneScopeAndStringConstructor(true, (TypeElement) e);
		return result;
	}
