...
/**
 * The SpeciesProcessor is responsible for processing {@code @species} annotations during the annotation processing
 * phase.
 *
 * <p>
 * Species in GAMA represent agent types - the fundamental building blocks of agent-based models. Each species defines a
 * template for creating agents with specific behaviors, attributes, and skills. The species processor handles the
 * registration of these agent types with the GAMA runtime system.
 *
 * <p>
 ...
 * <p>
 * A typical species definition includes:
 * <ul>
...
public class SpeciesProcessor extends ElementProcessor<species> {
	 ...
	 *
	 * <p>
	 * This method generates the runtime registration code for a GAMA species. The process involves:
	 * <ol>
	 ...
	 *
	 * <p>
	 * The generated code enables GAMA to:
	 * <ul>
	 ...
	 * @param e
	 *            the class element annotated with @species
	 * @param spec
	 *            the species annotation containing the species metadata
	 */
	@Override
	public void createElement(final StringBuilder sb, final Element e, final species spec) {
		...
		sb.append(in).append("_species(").append(toJavaString(spec.name())).append(",").append(toClassObject(clazz))
				.append(",(p, i)->").append("new ").append(getClassName(clazz)).append("(p, i),");
	 ...
	 *
	 * <p>
	 * This method ensures that the species class properly extends the IAgent interface, which is required for all agent
	 * types in GAMA. The IAgent interface provides the fundamental contract that all agents must implement, including:
	 * <ul>
	 ...
	 *
	 * @param e
	 *            the element to validate (should be a class annotated with @species)
	 * @return {@code true} if the element extends IAgent, {@code false} otherwise
	 */
	@Override
	protected boolean validateElement(final Element e) {
		return assertClassExtends(true, (TypeElement) e, context.getIAgent());
	}
