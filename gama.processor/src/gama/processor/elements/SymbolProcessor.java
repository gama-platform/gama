 ...
 *
 * <p>
 * Symbols in GAMA represent language constructs that can appear in GAML models - these include statements,
 * declarations, and other syntactic elements that form the building blocks of GAML code. Each symbol defines its
 * behavior, accepted facets, containment rules, and execution semantics.
 *
 * <p>
 * This processor handles the complex task of:
 * <ul>
 ...
 * <li><strong>Containment Rules:</strong> Processing {@code @inside} annotations that define where symbols can
 * appear</li>
 ...
 * <p>
 * A typical symbol definition includes:
 * <ul>
 ...
 * 			with_args = true)
 * 	&#64;inside (
 * 			symbols = { "experiment", "species", "action" })
 * 	&#64;facets (
 * 			value = { @facet (
 * 					name = "message",
 * 					type = IType.STRING,
...
public class SymbolProcessor extends ElementProcessor<symbol> {
	 ...
	 *
	 * <p>
	 * This method generates the runtime registration code for a GAMA symbol. The process involves:
	 * <ol>
	 ...
	 *
	 * <p>
	 * The generated code registers the symbol with all its metadata, enabling the GAMA parser to recognize and properly
	 * handle the symbol in GAML models.
	 *
	 * @param sb
	 *            the StringBuilder to append the generated registration code to
	 * @param e
	 *            the class element annotated with @symbol
	 * @param symbol
	 *            the symbol annotation containing the symbol's metadata
	 */
	@Override
	public void createElement(final StringBuilder sb, final Element e, final symbol symbol) {
		...
		toArrayOfStrings(symbol.name(), sb).append(',').append(toClassObject(clazz)).append(',').append(symbol.kind())
				.append(',').append(toBoolean(symbol.breakable())).append(',').append(toBoolean(symbol.continuable()))
				.append(',').append(toBoolean(symbol.remote_context())).append(',')
				.append(toBoolean(symbol.with_args())).append(',').append(toBoolean(symbol.with_scope())).append(',')
				.append(toBoolean(symbol.with_sequence())).append(',').append(toBoolean(symbol.unique_in_context()))
		...
		sb.append(',').append(toJavaString(omissible)).append(',').append("(x)->new ").append(getClassName(clazz))
				.append("(x)");
		sb.append(");");
	 ...
	 *
	 * <p>
	 * This method handles the {@code @facets} annotation which defines the parameters that a symbol accepts. It
	 * processes each facet definition and generates the appropriate runtime code for facet validation and handling.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * Facets with constant values are collected separately and added to the constants StringBuilder for later
	 * processing.
	 *
	 * @param sb
	 *            the StringBuilder to append the facets code to
	 * @param e
	 *            the symbol element (for error reporting)
	 * @param facets
	 *            the facets annotation containing all facet definitions
	 * @param constants
	 *            the StringBuilder for collecting constant definitions
	 ...
	 *
	 * <p>
	 * This method generates the registration code for an individual facet within a symbol. It handles all facet
	 ...
	 *
	 * @param sb
	 *            the StringBuilder to append the facet code to
	 * @param e
	 *            the symbol element (for error reporting and documentation validation)
	 * @param child
	 *            the individual facet annotation to process
	 * @param constants
	 *            the StringBuilder for collecting constant definitions from facet values
	 */
	private void processFacet(final StringBuilder sb, final Element e, final facet child,
		...
		toArrayOfInts(child.type(), sb).append(',').append(child.of()).append(',').append(child.index()).append(',');
	 ...
	 * <ol>
	 * <li><strong>Inheritance Check:</strong> Verifies that the symbol class extends the required ISymbol interface,
	 * ensuring it can participate in the GAMA symbol system</li>
	 * <li><strong>Annotation Check:</strong> Checks for the presence of {@code @inside} annotations which define
	 * containment rules (though this is not mandatory)</li>
	 * </ol>
	 *
	 * <p>
	 * Symbols must implement the ISymbol interface to be properly integrated into the GAMA language processing system
	 * and provide the required symbol behavior.
	 *
	 * @param e
	 *            the element to validate (should be a class annotated with @symbol)
