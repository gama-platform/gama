...
public class SymbolProcessor extends ElementProcessor<symbol> {
	...
	public void createElement(final StringBuilder sb, final Element e, final symbol symbol) {
		...
		toArrayOfStrings(symbol.name(), sb).append(',').append(toClassObject(clazz)).append(',')
				.append(symbol.kind().code()).append(',').append(toBoolean(symbol.breakable())).append(',')
				...
				.append(',').append(toBoolean(symbol.with_sequence())).append(',')
				.append(toBoolean(symbol.unique_in_context())).append(',').append(toBoolean(symbol.unique_name()))
				.append(',');
		...
		if (inside != null) {
			...
			toArrayOfInts((int[]) null, sb).append(',');
		}
