...
public abstract class SwingControl extends Composite {
	...
	if (PlatformHelper.isLinux()) {
	    return new SwingControlLinux(parent, view, surface, style);
	}
	if (PlatformHelper.isWindows()) {
	    return new SwingControlWin(parent, view, surface, style);
	}
	if (PlatformHelper.isMac()) {
	    return new SwingControlMac(parent, view, surface, style);
	}
	return null;
    }
    ...
    /**
     * A terrible hack to be able to dispose of the listeners installed by
     * SWT_AWT and wrongly removed (the shell it installs them on is not the
     * shell it removes them from)
     */
	...
	// DEBUG.OUT("-- SwingControl bounds set to " + x + " " + y + " | " +
	// width + " " + height);
	populate();
	// See Issue #3426
     ...
     */
    public void setMouseListener(final MouseMotionListener adapter) {
	swingMouseListener = adapter;
    }
