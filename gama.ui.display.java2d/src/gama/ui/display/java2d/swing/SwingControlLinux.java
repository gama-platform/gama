...
/**
 * Linux-specific implementation of {@link SwingControl} that embeds a {@link Java2DDisplaySurface} inside a SWT-AWT
 * bridge frame. The surface is added directly to the AWT {@link java.awt.Frame} (no intermediate Swing panel is needed,
 * as the surface manages its own layout via {@link java.awt.BorderLayout}). The mouse-wheel and mouse-event workaround
 * for Linux rendering artefacts (issue #2476) is installed on the frame via
 * {@link gama.ui.display.java2d.WorkaroundForIssue2476}.
 */
public class SwingControlLinux extends SwingControl {
	 ...
	 * @param view
	 *            the {@link AWTDisplayView} that owns this control, used by the parent class to track part-visibility
	 *            events
	 * @param component
	 *            the {@link Java2DDisplaySurface} (a {@link javax.swing.JPanel} subclass) that will be added to the
	 *            embedded Swing hierarchy
	 * @param style
	 ...
	 * <ol>
	 * <li>Creates an AWT {@link java.awt.Frame} via {@code SWT_AWT.new_Frame} and registers any key and mouse-motion
	 * listeners directly on it.</li>
	 * <li>Adds the {@link Java2DDisplaySurface} directly to the frame (no intermediate panel is required, as the
	 * surface manages its own {@link java.awt.BorderLayout}) and installs the Linux mouse workaround via
	 * {@link gama.ui.display.java2d.WorkaroundForIssue2476#installOn(java.awt.Container, gama.api.ui.displays.IDisplaySurface)}.</li>
	 * <li>Registers a {@code SWT.Dispose} listener that removes all listeners and properly disposes of the frame and
	 * surface when the SWT control is destroyed (see issue #489).</li>
	 * </ol>
