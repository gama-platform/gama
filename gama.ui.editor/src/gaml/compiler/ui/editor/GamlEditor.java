...
public class GamlEditor extends XtextEditor implements IGamlBuilderListener, IToolbarDecoratedView {
	...
	if (forceState || !state.equals(newState)) {
		...
		boolean showExperiments = !GamlFileExtension
			.isExperiment(getDocument().getAdapter(IFile.class).getName()) && newState.showExperiments;
    ...
    CreateExperimentSelectionListener listener;

    /**
     * @param showExperiments
     */
    private void updateAddExperimentButton(boolean showExperiments) {
	if (addExperiments == null) {
	    if (showExperiments) {
		addExperiments = GamaCommand
			.build("editor/add.experiment", null, "Add an experiment to the model", listener)
			.toItem(toolbar.getToolbar(SWT.LEFT));
	    } else {
		addExperiments = GamaCommand.build("editor/add.experiment", null, "No experiment can be added", null)
			.toItem(toolbar.getToolbar(SWT.LEFT));
	    }
	    toolbar.getToolbar(SWT.LEFT).space(8);
	} else if (showExperiments) {
	    addExperiments.setToolTipText("Add an experiment to the model");
	    addExperiments.addSelectionListener(listener);
	} else {
	    addExperiments.setToolTipText("No experiment can be added");
	    addExperiments.removeSelectionListener(listener);
	}
	...
	public IAutoEditStrategy[] getAutoEditStrategies(final ISourceViewer sourceViewer, final String contentType) {
	    ...
	    if (!GamaPreferences.Modeling.CORE_SURROUND_SELECTED.getValue()) {
		return strategies;
	    }
	    for (IAutoEditStrategy strategy : strategies) {
		if (strategy instanceof SurroundWithBracketsStrategy) {
		    return strategies;
		}
	    }
	...
	/** The empty. */
	static CompletableFuture<List<? extends ICodeMining>> EMPTY = CompletableFuture
		.completedFuture(Collections.EMPTY_LIST);

	@Override
	public CompletableFuture<List<? extends ICodeMining>> provideCodeMinings(final ITextViewer viewer,
		final IProgressMonitor monitor) {
	    if (!GamaPreferences.Modeling.EDITOR_MINING.getValue()) {
		return EMPTY;
	    }
	...
	if (sv.canDoOperation(ISourceViewer.FORMAT)) {
     ...
     */
    public static void setDiagramOpener(final IDiagramOpener opener) {
	diagramOpener = opener;
    }
	...
	if (sourceViewer.getDocument() != null) {
	    ...
	    StyledText styledText = sourceViewer.getTextWidget();
	    styledText.setFont(font);
	    if (getVerticalRuler() instanceof IVerticalRulerExtension e) {
		e.setFont(font);
	    }
	}
    }
