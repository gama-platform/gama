/*******************************************************************************************************
 *
 * ExperimentStartup.java, in gama.ui.experiment, is part of the source code of the GAMA modeling and
...
/**
 * Preloads expensive first-use resources shortly after workbench startup, so that the very first launch of a
 * simulation experiment is as fast as subsequent ones.
 *
 * <h2>What is pre-warmed and why</h2>
 * <ul>
 * <li><b>{@link WorkbenchHelper#SERVICES} cache</b> — {@link EModelService}, {@link EPartService},
 * {@link MApplication} and {@link ICommandService} are looked up on every call to
 * {@link gama.ui.experiment.commands.ArrangeDisplayViews}. The Guava {@code LoadingCache} calls
 * {@code IWorkbench.getService} on first access; pre-warming avoids that overhead on the critical
 * display-layout path.</li>
 ...
 * {@link org.eclipse.swt.graphics.Color} objects. Loading the class early ensures those allocations happen before
 * the first display view is painted.</li>
 * <li><b>{@link ThemeHelper#isDark()}</b> — triggers the CSS-engine theme detection, which can be slow on first
 * call.</li>
 * </ul>
 *
 * <p>
 * All work runs on a low-priority daemon thread, started with a short delay so the workbench finishes its own
 * startup first. SWT-resource creation (colors, images) is marshalled onto the UI thread via
 * {@link WorkbenchHelper#runInUI}.
 * </p>
 *
...
public class ExperimentStartup implements IStartup {
	...
	private static void prewarm() {
		...
		WorkbenchHelper.runInUI("Pre-warming simulation perspective", 0,
				m -> PerspectiveHelper.prewarmPerspective());

		// 3. Load all SVG icons into GamaIcon's cache — Image creation requires a Display (UI thread).
		WorkbenchHelper.runInUI("Pre-loading GAMA icons", 0, m -> {
			try {
				GamaIcon.preloadAllIcons();
			} catch (final Exception e) {
				DEBUG.ERR("ExperimentStartup: icon preload failed — " + e.getMessage());
			}
		});
