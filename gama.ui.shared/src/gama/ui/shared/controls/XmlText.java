...
public class XmlText extends Canvas {
	/**
	 * The object ID to be used when registering action to handle URL hyperlinks (those that should result in opening
	 * the web browser). Value is "urlHandler".
	 */
	...
	public XmlText(final Composite parent, final int style) {
		...
		addListener(SWT.KeyDown, e -> { if (e.character == '\r') { activateSelectedLink(); } });
		addListener(SWT.Traverse, e -> {
			...
			} else if (e.detail != SWT.TRAVERSE_RETURN) { e.doit = true; }
		});
		addFocusListener(new FocusListener() {
			...
			public void focusGained(final FocusEvent e) {
				if (!hasFocus) {
					hasFocus = true;
					if (!mouseFocus && !controlFocusTransfer) { handleFocusChange(); }
				}
			}
			...
			public void focusLost(final FocusEvent e) {
				if (hasFocus) {
					hasFocus = false;
					if (!controlFocusTransfer) { handleFocusChange(); }
				}
			}
		});
	 ...
	 *
	 * @return <samp>true </samp> if the widget is still loading the text, <samp>false </samp> otherwise.
	 ...
	 */
	@Deprecated
	public String getLoadingText() { return null; }

	/**
	 * Sets the text that will be shown in the control while the real content is loading. This is significant when
	 * content to render is loaded from the input stream that was created from a remote URL, and the time to load the
	 ...
	 * <p>
	 * For <samp>img </samp> tags, an object of a type <samp>Image </samp> must be registered using the key equivalent
	 * to the value of the <samp>href </samp> attribute used in the tag.
	 *
	 * @param key
	 *            unique key that matches the value of the <samp>href </samp> attribute.
	 * @param image
	 *            an object of a type <samp>Image </samp>.
	 */
	public void setImage(final String key, final Image image) {
	 ...
	 * <p>
	 * For <samp>span </samp> tags, an object of a type <samp>Color </samp> must be registered using the key equivalent
	 * to the value of the <samp>color </samp> attribute.
	 *
	 * @param key
	 *            unique key that matches the value of the <samp>color </samp> attribute.
	 * @param color
	 *            an object of the type <samp>Color </samp> or <samp>null</samp> if the key needs to be cleared.
	 */
	 ...
	 * <p>
	 * For <samp>span </samp> tags, an object of a type <samp>Font </samp> must be registered using the key equivalent
	 * to the value of the <samp>font </samp> attribute.
	 *
	 * @param key
	 *            unique key that matches the value of the <samp>font </samp> attribute.
	 * @param font
	 *            an object of the type <samp>Font </samp> or <samp>null</samp> if the key needs to be cleared.
	 */
	public void setFont(final String key, final Font font) {
	 ...
	 * <p>
	 * For <samp>control</samp> tags, an object of a type <samp>Control</samp> must be registered using the key
	 * equivalent to the value of the <samp>control</samp> attribute.
	 *
	 * @param key
	 *            unique key that matches the value of the <samp>control</samp> attribute.
	 * @param control
	 *            an object of the type <samp>Control</samp> or <samp>null</samp> if the existing control at the
	 *            specified key needs to be removed.
	 * @since 3.1
	 */
	public void setControl(final String key, final Control control) {
	...
	private void hookControlSegmentFocus() {
		...
		Listener listener = e -> {
			switch (e.type) {
				case SWT.FocusIn:
					if (!controlFocusTransfer) { syncControlSegmentFocus((Control) e.widget); }
					break;
				case SWT.Traverse:
					switch (e.detail) {
						case SWT.TRAVERSE_PAGE_NEXT:
					...
					} else if (e.detail == SWT.TRAVERSE_TAB_PREVIOUS) { e.doit = advanceControl(c, segment, false); }
		...
		for (Paragraph p : paragraphs) {
			...
			for (ParagraphSegment segment : segments) {
				if (segment instanceof ControlSegment cs) {
					...
					if (c != null && c.getData(CONTROL_KEY) == null) {
						// first time - hook
	 ...
	 *            the listener
	 */
	private void attachTraverseListener(final Control c, final Listener listener) {
		if (c instanceof Composite parent) {
			Control[] children = parent.getChildren();
			for (Control element : children) { attachTraverseListener(element, listener); }
	...
	/**
	 * If we click on the control randomly, our internal book-keeping will be off. We need to update the model and mark
	 * the control segment and currently selected. Hyperlink that may have had focus must also be exited.
	 *
	 * @param control
	...
	private void syncControlSegmentFocus(Control control) {
		...
		if (current instanceof IHyperlinkSegment) {
	 ...
	 */
	private boolean advanceControl(final Control c, ControlSegment segment, final boolean next) {
		...
		if (setFocusToNextSibling(c, next)) return true;
	...
	/**
	 * Sets the focus to next sibling.
	 *
	 * @param c
	 *            the c
	 * @param next
	 ...
	 * @return true, if successful
	 */
	private boolean setFocusToNextSibling(final Control c, final boolean next) {
		Composite parent = c.getParent();
			...
			if (child == c) {
				...
				if (next) {
					for (int j = i + 1; j < children.length; j++) {
						...
						if (nc.setFocus()) return false;
					}
				} else {
					for (int j = i - 1; j >= 0; j--) {
						Control pc = children[j];
						if (pc.setFocus()) return false;
					}
				}
			}
		}
	...
	private void computeSelection() {
		...
		for (Paragraph p : paragraphs) { p.computeSelection(gc, resourceTable, selectedLink, selData); }
		gc.dispose();
	}
	 ...
	 *            the segment
	 */
	private void updateTooltipText(final ParagraphSegment segment) {
	 ...
	 * @return true, if successful
	 */
	private boolean advance(final boolean next) {
		IFocusSelectable current = model.getSelectedSegment();
		if (current instanceof IHyperlinkSegment) { exitLink((IHyperlinkSegment) current, SWT.NULL); }
		...
		while (!valid) {
			if (!model.traverseFocusSelectableObjects(next)) { break; }
		...
		if (valid) { enterLink(newLink, SWT.NULL); }
		IHyperlinkSegment oldLink = current instanceof IHyperlinkSegment ? (IHyperlinkSegment) current : null;
		if (oldLink != null || newLink != null) { paintFocusTransfer(oldLink, newLink); }
		if (newLink != null) { ensureVisible(newLink); }
		if (newLink != null) { getAccessible().setFocus(model.getSelectedSegmentIndex()); }
		return !valid;
	}

	/**
	 * Sets the control focus.
	 *
	 * @param next
	 *            the next
	 * @param selectable
	 ...
	 * @return true, if successful
	 */
	private boolean setControlFocus(final boolean next, final IFocusSelectable selectable) {
		controlFocusTransfer = true;
	...
	private void handleFocusChange() {
		...
		if (!model.hasFocusSegments()) { redraw(); }
	}
	 ...
	 *            the hover
	 */
	private void paintLinkHover(final IHyperlinkSegment link, final boolean hover) {
		GC gc = new GC(this);
	...
	private void activateSelectedLink() {
		IHyperlinkSegment link = getSelectedLink();
		if (link != null) { activateLink(link, SWT.NULL); }
	}
	...
	private void repaint(final GC gc, final int x, final int y, final int width, final int height) {
		...
		IHyperlinkSegment selectedLink = getSelectedLink();
		if (getDisplay().getFocusControl() != this) { selectedLink = null; }
		for (Paragraph p : paragraphs) { p.paint(textGC, repaintRegion, resourceTable, selectedLink, selData); }
		if (hasFocus && !model.hasFocusSegments()) { textGC.drawFocus(x, y, width, height); }
		textGC.dispose();
	 ...
	 *            the new link
	 */
	private void paintFocusTransfer(final IHyperlinkSegment oldLink, final IHyperlinkSegment newLink) {
		if (oldLink != null) {
	 ...
	 *            the segment
	 */
	private void ensureVisible(final IFocusSelectable segment) {
		if (mouseFocus) {
			mouseFocus = false;
			return;
		}
	 ...
	 *
	 * @param disposeBoldFont
	 *            the dispose bold font
	 */
	private void disposeResourceTable(final boolean disposeBoldFont) {
		if (disposeBoldFont) {
			Font boldFont = (Font) resourceTable.get(FormTextModel.BOLD_FONT_ID);
			if (boldFont != null) {
				FormFonts.getInstance().markFinished(boldFont, getDisplay());
		...
		for (Enumeration<String> enm = resourceTable.keys(); enm.hasMoreElements();) {
			...
			if (key.startsWith(ImageSegment.SEL_IMAGE_PREFIX)) {
				...
				if (obj instanceof Image image && !image.isDisposed()) {
					image.dispose();
					imagesToRemove.add(key);
				}
			}
		}
		for (String element : imagesToRemove) { resourceTable.remove(element); }
	}
