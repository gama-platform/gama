...
public abstract class AbstractEditor<T> implements Selector, ModifyListener, IParameterEditor<T> {
	...
	agent = a;
	if (agent == null) {
	    throw GamaRuntimeException.error("The parameters view cannot be opened.", a.getScope());
	}
	...
	if (o != null) {
	    minValue = castValueToInnerType(o);
	}
	o = param.getMaxValue(getScope());
	if (o != null) {
	    maxValue = castValueToInnerType(o);
	}
	o = param.getStepValue(getScope());
	if (o != null) {
	    stepValue = castValueToInnerType(o);
	}
	...
	if (isEditable) {
	    if (isCombo) {
		editorControl = new ComboEditorControl(this, composite, getExpectedType(),
			param.getAmongValue(getScope()));
	    } else {
	...
	if (editorToolbar != null && !editorToolbar.isDisposed()) {
