...
public class FileEditor extends AbstractEditor<IGamaFile> {
	...
	public Control createCustomParameterControl(final Composite comp) {
		...
		textBox.setSelectionListener(this);
		return textBox;
	}
	...
	public void widgetSelected(final SelectionEvent e) {
		...
		if (isWorkspace) {
			...
			dialog.setFilterPath(file != null ? file.getPath(getScope())
					: GAMA.getModel() == null ? GAMA.getWorkspaceManager().getWorkspaceLocation()
			...
			dialog.setFilterPath(file != null ? file.getPath(getScope())
					: GAMA.getModel().getFilePath().isBlank() ? GAMA.getWorkspaceManager().getWorkspaceLocation()
					: GAMA.getModel().getFilePath());
	...
	static public class WorkspaceResourceDialog extends ElementTreeSelectionDialog
		...
		public void loadContents() {
			IWorkspaceRoot root = GAMA.getWorkspaceManager().getRoot();
			setInput(root);
		}
