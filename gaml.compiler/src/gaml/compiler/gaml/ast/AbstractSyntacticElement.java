 ...
 * 
 * <p>This class provides the core implementation for representing parsed GAML language constructs
 * during the compilation process. Each syntactic element corresponds to a language construct like
 * species, experiments, statements, or expressions.</p>
 * 
 ...
 * </ul>
 * 
 * <p><strong>Thread Safety:</strong> This class is NOT thread-safe. Instances should be accessed
 * from a single thread or externally synchronized during AST construction and traversal.</p>
 * 
...
public abstract class AbstractSyntacticElement implements ISyntacticElement {
	...
	/**
	 * The facets (attributes) of this element, stored as key-value pairs.
	 * Lazily initialized and set to null when empty to optimize memory usage.
	 */
	 ...
	 * This provides access to the original source location and structure.
	 * Made final as it should never change after construction.
	 */
	 ...
	 * 
	 * <p>This constructor is package-private and should only be called by subclasses
	 * through the {@link SyntacticFactory}.</p>
	 *
	 * @param keyword the GAML keyword identifying this element type (e.g., "species", "experiment").
	 *                Must not be null for proper element identification.
	 * @param facets  the initial facets (attributes) for this element, or null if none.
	 *                Facets will be lazily initialized on first use if null.
	 * @param element the underlying EMF EObject from the parsed model. Should not be null
	 *                as it provides source location information for error reporting.
	 */
	 ...
	 * 
	 * <p>The EObject provides access to the original source structure and location,
	 * which is essential for error reporting and debugging.</p>
	 *
	 * @return the EMF EObject representing this element in the parsed model, may be null
	 ...
	 * 
	 * <p>The format is: {@code <keyword> <name> <facets>}</p>
	 *
	 ...
	 * 
	 * <p>The default implementation throws a RuntimeException as base elements
	 * do not support children. Override in subclasses like {@link SyntacticComposedElement}
	 * to enable child management.</p>
	 *
	 * @param e the child element to add
	 * @throws RuntimeException always, as this element type doesn't support children
	 */
	@Override
	 ...
	 * 
	 * <p>Use with caution as changing the keyword may affect how the element
	 * is processed during compilation.</p>
	 *
	 * @param name the new keyword to set
	 */
	@Override
	 ...
	 * 
	 * <p>This is an optimized check that doesn't create the facets collection
	 * if it doesn't exist yet.</p>
	 *
	 ...
	 * 
	 * <p>This method performs a null-safe check without initializing the facets
	 * collection if it doesn't exist.</p>
	 *
	 ...
	 * 
	 * <p>Facets represent attributes like name, type, value, etc. This method
	 * provides safe access without throwing exceptions for missing facets.</p>
	 *
	 * @param name the facet name to retrieve
	 ...
	 * 
	 * <p>This method is essential during AST transformation and validation. It creates
	 * clean copies of expression descriptions to avoid shared mutable state between
	 * syntactic and semantic models.</p>
	 * 
	 * <p>Label facets are specially handled: they are compiled as labels which affects
	 * how they are validated and resolved during compilation.</p>
	 *
	 * @param sp the symbol prototype defining which facets are labels, or null to skip
	 *           label processing
	 ...
	 * 
	 * <p>Facets are lazily initialized - the collection is only created when the first
	 * facet is set. This reduces memory usage for simple elements.</p>
	 *
	 * @param string the facet name
	 * @param expr   the expression description for the facet value, ignored if null
	 */
	@Override
	 ...
	 * 
	 * <p>Default implementation extracts the name from the "name" facet.
	 * Subclasses may override to cache the name or compute it differently.</p>
	 *
	 ...
	 * 
	 * <p>If removing the facet results in an empty collection, the facets object
	 * is nullified to save memory.</p>
	 *
	 ...
	 * 
	 * <p>Used during AST traversal to identify and process species elements separately.</p>
	 *
	 * @return false by default, overridden to return true in {@link SyntacticSpeciesElement}
	 */
	@Override
	 ...
	 * 
	 * <p>Experiments are special top-level constructs that define simulation scenarios.</p>
	 *
	 * @return false by default, overridden to return true in {@link SyntacticExperimentElement}
	 */
	@Override
	 ...
	 * 
	 * <p>This method recursively visits all children and increments counters in the
	 * provided map. Used for AST analysis, profiling, and debugging.</p>
	 * 
	 * <p><strong>Optimization Note:</strong> Uses {@code computeIfAbsent} for cleaner code
	 * and better performance with modern JVMs.</p>
	 *
	 * @param stats a mutable map from class simple name to occurrence count
	 */
	@Override
	 ...
	 * 
	 * <p>The visitor pattern allows processing facets without exposing the internal
	 * facets collection. The visitor can terminate early by returning false.</p>
	 *
	 * @param visitor the facet visitor to apply to each facet
	 */
	@Override
	 ...
	 * 
	 * <p>This memory optimization should be called after AST construction is complete
	 * and no more facets will be added. The {@link SyntacticModelElement#compactModel()}
	 * method applies this recursively to the entire tree.</p>
	 */
	@Override
	public void compact() {
		if (facets == null) return;
	 ...
	 * 
	 * <p>Default implementation only visits this element. Composed elements override
	 * to recursively visit children.</p>
	 *
	 * @param visitor the visitor to apply to each element in the tree
	 */
	@Override
	 ...
	 * 
	 * <p>Default implementation does nothing as base elements have no children.
	 * Override in subclasses to implement child visitation.</p>
	 *
	 ...
	 * 
	 * <p>Species are structural elements that define agent types. This method
	 * allows selective traversal of species definitions.</p>
	 *
	 ...
	 * 
	 * <p>Experiments are typically found at the model level and define
	 * simulation execution scenarios.</p>
	 *
	 ...
	 * 
	 * <p>Grids are special species that represent spatially organized agent populations.
	 * This method allows separate processing of grid definitions.</p>
	 *
	 ...
	 * 
	 * <p>This method establishes the standard traversal order for AST processing.
	 * Note that experiments are intentionally excluded from this traversal as they
	 * are processed separately.</p>
	 *
	 * @param visitor the visitor to apply to each child
	 */
	@Override
	 ...
	 * 
	 * <p>This method should be called when the element is no longer needed to allow
	 * garbage collection. It disposes the facets collection but doesn't modify
	 * the EObject reference.</p>
	 */
	@Override
	public void dispose() {
	 ...
	 * 
	 * <p>Default implementation returns null. Overridden in {@link SyntacticModelElement}
	 * to return the model's file path.</p>
	 *
