/*******************************************************************************************************
 *
 * PrimitiveDescription.java, in gaml.compiler, is part of the source code of the GAMA modeling and simulation platform
 * (v.2025-03).
 *
...
public class PrimitiveDescription extends ActionDescription {
	...
	public String getBuiltInDoc() {
		...
		if (d == null) {
			...
			s = d.value() + StringUtils.LN;
		} else {
			s = d.value() + StringUtils.LN + StringUtils.LN + d.deprecated() + StringUtils.LN;
		}
		return s;
	}
	...
	public IType<?> computeType() {
		...
		Integer t = Integer.valueOf(getLitteral(IKeyword.TYPE));
		return Types.get(t);
	}
