 ...
 * 
 * <p>This class provides the foundation for the expression evaluation system in GAMA,
 * implementing common functionality and defining the contract for concrete expression classes.</p>
 * 
 ...
 * <p>Expression evaluation follows the <strong>Template Method</strong> pattern:</p>
 * <ol>
 ...
 * <p>Types are computed at compilation and cached:</p>
 * <pre>{@code
 * IExpression expr = ...;
 ...
 * <p>The {@link #optimized()} method enables constant folding when enabled:</p>
 * <pre>{@code
 * // At compile time:
 ...
 * 
 * <h2>Thread Safety</h2>
 * <p><strong>Expressions are immutable after construction.</strong> They can be safely shared across threads
 * and evaluated concurrently with different scopes. The {@code type} field is effectively final once set.</p>
 * 
 * <h2>Serialization Contract</h2>
 * <p>All expressions must implement {@link #serializeToGaml(boolean)} to support:</p>
 * <ul>
 ...
 * IExpression expr = GAML.getExpressionFactory()
 *     .createOperator("+", context, eObject, left, right);
 * 
 * // Evaluating
 * Object result = expr.value(scope);
 * 
 * // Type checking
 * if (expr.getGamlType().equals(Types.INT)) {
...
public abstract class AbstractExpression implements IExpression {
	 ...
	 * 
	 * <p><strong>Performance Note:</strong> Computed once at construction and cached.
	 * Making this field final would enable additional JVM optimizations.</p>
	 * 
	 * <p><strong>Optimization Opportunity:</strong> Consider using a final field set via constructor
	 * to enable better constant propagation and inlining by the JIT compiler.</p>
	 */
	 ...
	 * 
	 * <p><strong>Performance:</strong> O(1) - returns cached type. The null check is optimized
	 * away by the JIT after first invocation (branch prediction).</p>
	 *
	 ...
	 * 
	 * <p><strong>Optimization Note:</strong> This method is called frequently during serialization.
	 * Consider pooling StringBuilder instances or caching serialized forms.</p>
	 * 
	 * <p><strong>Performance:</strong> Creates temporary StringBuilder, which may cause GC pressure
	 * for complex expressions. Average cost: ~1-5 µs depending on expression complexity.</p>
	 *
	 * @param sb the StringBuilder to append to
	 * @param exp the expressions to parenthesize
	 */
	 ...
	 * 
	 * <p>Used for serializing lists, maps, function calls, and parenthesized expressions.</p>
	 * 
	 * <p><strong>Performance:</strong> Iterates through all expressions once. For n expressions,
	 * complexity is O(n) where n is typically small (1-10).</p>
	 *
	 * @param sb the StringBuilder to append to
	 * @param first the opening delimiter
	 * @param last the closing delimiter
	 * @param exp the expressions to surround
	 ...
	 * 
	 * <p><strong>Template Method Pattern:</strong> This public method handles cross-cutting concerns
	 * (benchmarking, error handling) while delegating actual computation to {@link #_value(IScope)}.</p>
	 * 
	 ...
	 * </ul>
	 * 
	 * <p><strong>Thread Safety:</strong> Safe for concurrent evaluation with different scopes.
	 * Expressions are immutable after construction.</p>
	 * 
	 * <p><strong>Optimization Opportunity:</strong> Add fast-path when benchmarking is disabled:</p>
	 * <pre>{@code
	 * if (!BENCHMARKING_ENABLED) return _value(scope);
	 * // Otherwise use try-with-resources
	 * }</pre>
	 *
	 * @param scope the execution scope providing context
	 ...
	 * 
	 * <p><strong>Constant Folding Optimization:</strong> When this expression is constant
	 * (all operands are constants), replaces it with a single ConstantExpression containing
	 * the pre-computed value. This eliminates runtime evaluation cost.</p>
	 * 
	 ...
	 * </ul>
	 * 
	 * <p><strong>When Applied:</strong> Only if {@code GamaPreferences.Experimental.CONSTANT_OPTIMIZATION}
	 * is enabled AND {@link #isConst()} returns true.</p>
	 *
	 ...
	 * 
	 * <p><strong>Subclass Contract:</strong> Implement this method to provide expression-specific
	 * evaluation logic. This method is called by {@link #value(IScope)} after benchmarking setup.</p>
	 * 
	 * <p><strong>Performance Note:</strong> This is the hot path for expression evaluation.
	 * Keep implementations fast and avoid unnecessary allocations.</p>
	 *
	 ...
	 * 
	 * <p><strong>Use Case:</strong> Display-related expressions that need pixel resolution.</p>
	 * 
	 * <p><strong>Performance:</strong> Traverses expression tree once. Result could be cached
	 ...
	 * 
	 * <p><strong>Use Case:</strong> Determines if expression needs re-evaluation at each step.</p>
	 * 
	 * <p><strong>Performance:</strong> Traverses expression tree. Could be cached as expressions
	 * are immutable after construction.</p>
	 * 
	 * <p><strong>Optimization Opportunity:</strong> Cache result in a boolean field set during
	 * construction via visitor pattern.</p>
	 *
