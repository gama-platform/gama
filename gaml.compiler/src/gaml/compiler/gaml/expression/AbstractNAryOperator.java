/*******************************************************************************************************
 *
 * AbstractNAryOperator.java, in gama.api, is part of the source code of the GAMA modeling and simulation platform
 * (v.2025-03).
 *
...
import static gama.annotations.support.ITypeProvider.ALL;
...
import static gama.annotations.support.ITypeProvider.INDEXED_TYPES;
import static gama.annotations.support.ITypeProvider.KEY_TYPE_AT_INDEX;
import static gama.annotations.support.ITypeProvider.SECOND_CONTENT_TYPE_OR_TYPE;
import static gama.annotations.support.ITypeProvider.SECOND_DENOTED_TYPE;
import static gama.annotations.support.ITypeProvider.THIRD_CONTENT_TYPE_OR_TYPE;
...
import gama.api.gaml.expressions.IOperator.IOperatorVisitor;
...
public abstract class AbstractNAryOperator extends AbstractExpression implements IOperator {
	...
	protected IType computeType() {
		...
		if (result.isContainer()) {
			final IType contentType = computeType(prototype.getContentTypeProvider(),
					prototype.getContentTypeContentTypeProvider(), result.getContentType(), GamaType.CONTENT);
			final IType keyType = computeType(prototype.getKeyTypeProvider(), 0, result.getKeyType(), GamaType.KEY);
