 ...
 *
 * <p>
 * This validator is responsible for the complete validation pipeline of GAML resources, including:
 * <ul>
 ...
 *
 * <p>
 * The validation process respects cancellation requests through the {@link CancelIndicator} and tracks the total
 * validation duration for performance monitoring purposes.
 *
...
public class GamlResourceValidator implements IResourceValidator {
	...
	/**
	 * Tracks the cumulative time spent in validation operations across all resources. Used for performance monitoring
	 * and profiling. Value is in nanoseconds.
	 */
	...
	/**
	 * Converts EMF diagnostics into Xtext issues that can be displayed in the IDE. Injected by the dependency injection
	 * framework.
	 */
	@Inject IDiagnosticConverter converter;

	/**
	 * Translates GAML validation errors from the resource's validation context into EMF diagnostics. This translator
	 * bridges the gap between GAML's internal error representation and the standard EMF diagnostic model used by Xtext
	 * and Eclipse.
	 */
	 ...
	 *
	 * <p>
	 * This method performs a complete validation pipeline:
	 * <ol>
	 ...
	 *
	 * <p>
	 * The validation respects cancellation requests at multiple points to allow early termination of long-running
	 * validations.
	 *
	 * @param resource
	 *            the GAML resource to validate (must be a {@link GamlResource})
	 * @param mode
	 *            the validation mode (e.g., NORMAL_AND_FAST, ALL)
	 * @param indicator
	 *            cancellation indicator to check for user cancellation requests, may be null
	 * @return a list of validation issues found, or an empty list if validation was cancelled or the resource is not a
	 *         valid GamlResource
	 */
	@Override
	public List<Issue> validate(final Resource resource, final CheckMode mode, final CancelIndicator indicator) {
		...
		if (indicator != null && indicator.isCanceled() || !(resource instanceof GamlResource r)) return List.of();
	 ...
	 *
	 * <p>
	 * Displays a visual indicator that the specified file is being compiled. If the GUI is not available (e.g., in
	 * headless mode), the exception is silently ignored to allow validation to continue.
	 *
	 * @param name
	 ...
	 *
	 * <p>
	 * These are low-level errors that occur during parsing and cross-reference resolution, such as:
	 * <ul>
	 ...
	 *
	 * @param resource
	 *            the resource whose errors should be collected
	 * @param acceptor
	 *            the acceptor that receives converted issue objects
	 */
	 ...
	 *
	 * <p>
	 * These are high-level errors detected by GAML's semantic validation rules, such as:
	 * <ul>
	 ...
	 *
	 * <p>
	 * The validation context contains all errors accumulated during the resource's {@link GamlResource#validate()}
	 * execution. These errors are translated from GAML's internal representation to EMF diagnostics, then converted to
	 * Xtext issues.
	 *
	 * @param resource
	 *            the GAML resource whose validation errors should be collected
	 * @param mode
	 *            the validation mode that determines which validators to run
	 * @param acceptor
	 *            the acceptor that receives converted issue objects
	 */
