#!/usr/bin/env python3
"""fix_init_assignment.py

Replaces all occurrences of ``init:`` (with optional preceding whitespace)
by ``<-`` in GAML source files.

Examples
--------
    int a init: 5;    →  int a <- 5;
    string b init : ""; →  string b <- "";

Usage
-----
    # Dry-run (preview changes, no files modified)
    python3 fix_init_assignment.py /path/to/workspace --dry-run

    # Apply changes in-place
    python3 fix_init_assignment.py /path/to/workspace

    # Restrict to a specific file extension (default: .gaml, .experiment)
    python3 fix_init_assignment.py /path/to/workspace --ext .gaml
"""
...
# Matches: "init:" or "init :" (with optional whitespace before the colon)
_PATTERN = re.compile(r'\binit\s*:')
_REPLACEMENT = '<-'
...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description='Replace "init:" with "<-" in GAML files.')
