#!/usr/bin/env python3
"""fix_transition_to.py

Replaces all occurrences of ``transition to:`` (with any surrounding
whitespace) by ``transition `` in GAML source files.
...
-----
    ...
    python3 fix_transition_to.py /path/to/workspace --dry-run

    # Apply changes in-place
    python3 fix_transition_to.py /path/to/workspace

    # Restrict to a specific file extension (default: .gaml)
    python3 fix_transition_to.py /path/to/workspace --ext .gaml
"""
...
# Matches: transition  to:   (any amount of whitespace around "to:")
_PATTERN = re.compile(r'\btransition\s+to:\s+')
_REPLACEMENT = 'transition '
...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description='Replace "transition to:" with "transition " in GAML files.')
